...
- Before approving a secondary urban filler day, compare it internally with the strongest remaining
  feasible day trip or regional signature experience from the same base.
- Choose the day trip only when it clearly offers greater tourism value, uniqueness and traveler fit,
  and remains realistic for the actual date, daylight, transport, distance, safety and daily window.
- Do not force a fixed number of day trips and do not replace core city must-sees; this comparison
  applies only against the weakest remaining non-essential urban day.
...
- Every row interval must be mathematically accounted for internally by the real movement time plus the real activity dwell that actually apply. Preserve the strict temporal discipline of the itinerary even though the traveler-facing fields keep movement and activity separate. Never fabricate one merely because the other exists.
...
- Pure transfers and returns use kind "transport". Internally account for the full interval as movement time. Put the mode and its approximate door-to-door time together in transport (for example "Walk · ~15m" or "Bus · ~25m"); duration may be empty. Never invent an Activity duration for a movement-only row.
- A real visit/experience uses kind "activity". Before emitting the row, verify that movement time + activity dwell plausibly accounts for start→end. Put genuine access/mobility and its approximate time together in transport. Put only the actual dwell in duration as "Activity: <realistic estimate or range>". If no meaningful movement is needed, transport may be empty.
- Every usable planning window must be planned on its own calendar date. Never shift an arrival-evening sequence to the following day or treat a prior-day arrival as if it occurred today. A materially usable morning/afternoon window may not be left empty merely because the previous day ended with a transfer.
...
- Keep transport and activity duration mathematically consistent with start/end times. Internally verify movement + dwell = the row interval (within normal planning tolerance), even though movement time is displayed in transport and only dwell is displayed in duration. Cover every supplied planning_window on its exact date; never carry an arrival state into the next date.
