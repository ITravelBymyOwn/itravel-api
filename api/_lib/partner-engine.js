...
function omioCatalogRoute(origin,destination,locale='en'){
  ...
  // V61 definitive matcher: city names generated by the Workspace are compared
  // against the exact bilingual city pair in the official feeds FIRST. Position
  // IDs are intentionally only a fallback because a city label can legitimately
  // map to city/station/airport IDs and previously turned one valid A→B into
  // several matches, causing the resolver to reject the route as ambiguous.
...
async function resolveOmioExplicitRoutes(tripId,userId,city,uiLanguage,needs=[],transportRoutes=[],debug=[]){
  ...
  for(const raw of transportRoutes.slice(0,32)){
    ...
    const row={stage:'feed_lookup',index:raw?.is_parent?0:Number(raw?.index||1),need_id:clean(raw?.need_id,120),origin_raw:originRaw,destination_raw:destinationRaw,origin,destination,locale:localeResolution.locale,match:false,route_id:'',reason:''};
...
export async function lookupOmioFeedRoutes({ city = '', language = 'es', ui_language = '', transport_routes = [] }) {
  ...
  for (const raw of routes) {
    ...
    const key=`${clean(raw?.need_id,120)}|${raw?.is_parent?'parent':Number(raw?.index||1)}|${normalizeKey(origin)}|${normalizeKey(destination)}`;
    ...
    matches.push({
      id:`omio-direct:${row.route_id}:${raw?.is_parent?'parent':Number(raw?.index||1)}`,
      ...
      route_segment:{index:raw?.is_parent?0:Number(raw?.index||1),mode:clean(raw?.mode,40),commercial_origin:originRaw||origin,commercial_destination:destinationRaw||destination,parent_origin:clean(raw?.parent_origin,120),parent_destination:clean(raw?.parent_destination,120),is_parent:!!raw?.is_parent},
