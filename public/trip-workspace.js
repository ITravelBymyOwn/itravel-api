...
function visibleTransport(value,row={}){
  let raw=String(value||'').trim();if(!raw)return'';
  const canonicalFrom=normalizeWorkspaceEntity(row?.from||'');
  raw=raw.replace(/\b(desde|from)\s+(?:la\s+zona\s+de\s+|el\s+entorno\s+de\s+)?([^;,·]+)(?=[;,·]|$)/ig,(match,_prep,claimed)=>{
    const claimedKey=normalizeWorkspaceEntity(claimed||'');
  ...
  return raw.replace(/\s+([;,·])/g,'$1').replace(/^[\s,;·-]+|[\s,;·-]+$/g,'').replace(/\s{2,}/g,' ').trim();
}
function renderItinerary(){const rows=data?.itineraries?.[city]?.byDay?.[day]||[],b=base(city),date=b?fmt(addDays(b,day-1)):'',ds=days(city),idx=ds.indexOf(day);let html=`<div class="tw-day-header"><div><span class="tw-kicker">${esc(city)}</span><h2>${lang==='es'?'Día':'Day'} ${day}${date?` · ${esc(date)}`:''}</h2></div><span class="tw-day-count">${idx+1} / ${ds.length}</span></div><div class="tw-timeline">`;rows.forEach((r,i)=>{const activity=String(r.activity||'').replace(/^rev:\s*/i,''),notes=String(r.notes||'').replace(/^\s*valid:\s*/i,'').trim(),route=[r.from,r.to].filter(Boolean).join(' → '),transport=visibleTransport(r.transport,r);html+=`<article class="tw-stop"><div class="tw-time">${esc(r.start||'')}<small>${esc(r.end||'')}</small></div><div class="tw-node"></div><div class="tw-stop-card"><h3>${esc(activity)}</h3><div class="tw-pills">${transport?`<span class="tw-pill">${esc(transport)}</span>`:''}${r.duration?`<span class="tw-pill">${esc(cleanDuration(r.duration))}</span>`:''}</div>${(route||notes)?`<button class="tw-details-btn" type="button" data-detail="${i}">${esc(t.details)} ＋</button><div class="tw-details" id="tw-detail-${i}" hidden>${route?`<div class="tw-detail"><small>${esc(t.route)}</small><b>${esc(route)}</b></div>`:''}${transport?`<div class="tw-detail"><small>${esc(t.transport)}</small><b>${esc(transport)}</b></div>`:''}${r.duration?`<div class="tw-detail"><small>${esc(t.duration)}</small><b>${esc(cleanDuration(r.duration))}</b></div>`:''}${notes?`<div class="tw-detail"><small>${esc(t.notes)}</small><b>${esc(notes)}</b></div>`:''}</div>`:''}</div></article>`});html+='</div>';$('#tw-content').innerHTML=html;document.querySelectorAll('[data-detail]').forEach(btn=>btn.onclick=()=>{const box=$(`#tw-detail-${btn.dataset.detail}`),open=!box.hidden;box.hidden=open;btn.textContent=(open?t.details:t.hide)+(open?' ＋':' −')})}
function tripRoutes(){
  const routes=[];
  const seen=new Set();
  const add=route=>{
...
function isRentalTransportNeed(item){
  if(item?.need_type!=='transport_arrangement') return false;
  return rentalTransportTextMatches(`${item?.entity_name||''} ${item?.source_activity||''} ${item?.source_route||''} ${item?.transport||''} ${item?.user_message||''}`);
}
function contextDayOrder(item){
  if(item?.scope_label) return -1;
  const n=Number(item?.day);
  return Number.isFinite(n) && n>0 ? n : Number.MAX_SAFE_INTEGER;
}
function sortContextItems(items=[]){
  return [...items].sort((a,b)=>{
    const dayDiff=contextDayOrder(a)-contextDayOrder(b);
...
function contextualNeedsForCity(cityName,needs){
  ...
  const withoutDuplicateTopLevelRoutes=source.filter(item=>{
    if(rentalPlan && isRentalTransportNeed(item)) return false;
    if(item?.need_type!=='intercity_transport' && item?.need_type!=='transport_arrangement') return true;
...
function sectionSort(items=[],sectionType=''){
  const ordered=[...items];
  const priority=item=>{
...
function contextSection(icon,title,description,items,offers=[],sectionId='',sectionType=''){
  if(!items.length)return'';
  const id=sectionId?` id="${esc(sectionId)}"`:'';
  const ordered=sectionSort(items,sectionType);
  const sectionKey=`${city||''}:${sectionId||sectionType}`;
  const expanded=expandedContextSections.has(sectionKey);
  const initialLimit=initialSectionLimit(ordered,sectionType);
  const visibleCount=expanded?ordered.length:initialLimit;
  const hiddenCount=Math.max(0,ordered.length-visibleCount);
  const toggle=ordered.length>initialLimit
    ? `<button class="tw-context-toggle" type="button" data-context-toggle="${esc(sectionKey)}" aria-expanded="${expanded?'true':'false'}">${esc(expanded?t.showLess:`${t.showMore} (${hiddenCount})`)}</button>`
    : '';
  return `<section class="tw-context-section"${id}>
    ...
    ${renderNeedItems(ordered,offers,visibleCount)}
    ${toggle}
  </section>`;
}
async function fetchContext(cityName){
  ...
  const response=await fetch('/api/context',{
    ...
    body:JSON.stringify({
      ...
      city:cityName,
      ui_language:lang,
      trip_language:data?.trip_language||''
    })
  });
...
function renderPrepare(){
  ...
  const activeSections=[
    ...
    contextSection('↗',t.move,t.moveC,transport,transportOffers,'tw-context-move','transport')
  ].filter(Boolean).join('');
  ...
  document.querySelectorAll('[data-context-toggle]').forEach(button=>{
    button.onclick=()=>{
      const key=button.dataset.contextToggle||'';
      if(!key)return;
      if(expandedContextSections.has(key)) expandedContextSections.delete(key);
      else expandedContextSections.add(key);
      renderPrepare();
      requestAnimationFrame(()=>[...document.querySelectorAll('[data-context-toggle]')].find(el=>el.dataset.contextToggle===key)?.focus());
    };
  });
  bindPartnerOffers();
...
function tripToWorkspace(trip){
  ...
  const destinations=(Array.isArray(trip.destinations) ? trip.destinations : []).map(d=>({
    ...
    countryCode:String(d?.country_code||d?.countryCode||'').trim().toUpperCase(),
  ...
  const plannerState=(checkpoint.planner_state && typeof checkpoint.planner_state==='object')
    ? checkpoint.planner_state
    : {};
  const tripLanguage=normalizeTripLanguage(plannerState.itineraryLang||plannerState.itinerary_lang||'');
...
function setupPrepareFloating(){
  ...
  const floating=$('#tw-floating-prepare');
  if(!source||!floating)return;

  const update=()=>{
    if($('#tw-city')?.hidden || mode==='prepare'){
      floating.hidden=true;
      floating.classList.remove('is-visible');
      return;
    }
  ...
  floating.onclick=()=>{
    ...
    floating.hidden=true;
    floating.classList.remove('is-visible');
    window.ITBMOFoundation?.track('floating_prepare_clicked',{destination:city||'',language:lang,trip_id:data?.trip_id||''});
  ...
  window.addEventListener('tw:city-entered',()=>requestAnimationFrame(update));
  window.addEventListener('tw:overview-opened',update);
  window.addEventListener('tw:prepare-mode-changed',update);
  requestAnimationFrame(update);
}
...
async function boot(){
  ...
  setupAllCitiesFloating();setupPrepareFloating();
  overview();
