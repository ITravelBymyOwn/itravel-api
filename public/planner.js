...
function openTripStoryBuilder(){
  ...
  let phase='travelers', activeStay=0, pendingNextStay=null;
  ...
  overlay.innerHTML=`<div class="guided-journey" role="dialog" aria-modal="true" aria-label="${es?'Crea tu viaje':'Build your trip'}">
    ...
    <nav class="guided-journey__progress" aria-label="${es?'Progreso':'Progress'}"><span data-stage="travelers">${es?'Viajeros':'Travelers'}</span><i>›</i><span data-stage="route">${es?'Ruta':'Route'}</span><i>›</i><span data-stage="personalize">${es?'Personalización':'Personalization'}</span><i>›</i><span data-stage="itinerary">${es?'Itinerario':'Itinerary'}</span></nav>
    <div class="guided-journey__layout"><main class="guided-journey__active" data-gj-active></main><aside class="guided-journey__story"><div class="guided-journey__story-head"><div><small>${es?'TU RECORRIDO':'YOUR JOURNEY'}</small><b>${es?'Tu viaje toma forma aquí':'Your trip takes shape here'}</b></div><button type="button" data-gj-copy>${es?'Copiar recorrido':'Copy journey'}</button></div><div data-gj-story></div></aside></div>
  ...
  const render=()=>{stage();renderStory();
    ...
    if(phase==='stay'){
      ...
      bindLocation(active.querySelector('[data-stay-country]'),x=>{st.country=x.label;if(x.code)st.countryCode=x.code;persist();},'country');bindLocation(active.querySelector('[data-stay-place]'),x=>{st.place=x.label;persist();},'city',()=>st.countryCode);active.querySelector('[data-stay-start]').onchange=e=>{st.startDate=e.target.value;st.arrivalDate=st.arrivalDate||st.startDate;_tripStoryEnsureStay_(st);persist();renderStory();};active.querySelector('[data-stay-days]').onchange=e=>{st.days=Number(e.target.value);_tripStoryEnsureStay_(st);persist();renderStory();};active.querySelector('[data-gj-next]').onclick=()=>{if(!st.place||!st.startDate)return;phase=activeStay>0?'movement':'decision';render();};return;
    }
    if(phase==='decision'){
      const st=story.stays[activeStay];
      active.innerHTML=shell(es?'TU VIAJE SIGUE TOMANDO FORMA':'YOUR TRIP KEEPS TAKING SHAPE',_tripStoryEsc_(st.place),es?'¿Qué quieres hacer ahora?':'What would you like to do now?',`<div class="gj-choice-stack"><button type="button" data-add-daytrip class="gj-choice"><span>↗</span><div><b>${es?'Agregar excursión':'Add a day trip'}</b><small>${es?'Sales y regresas a este destino el mismo día.':'Leave and return to this destination the same day.'}</small></div></button><button type="button" data-add-next class="gj-choice"><span>＋</span><div><b>${es?'Agregar siguiente destino':'Add next destination'}</b><small>${es?'Continuaremos construyendo tu recorrido.':'Keep building your route.'}</small></div></button><button type="button" data-finish class="gj-choice"><span>✓</span><div><b>${es?'Mi recorrido termina aquí':'My journey ends here'}</b><small>${es?'Después podrás agregar el regreso, si lo conoces.':'You can add your return afterward if you know it.'}</small></div></button></div>${(st.dayTrips||[]).length?`<div class="gj-existing"><b>${es?'Excursiones':'Day trips'}</b>${st.dayTrips.map((d,i)=>`<div><span>↗ ${_tripStoryEsc_(d.place||'—')} · ${es?'Día':'Day'} </span><select data-move-daytrip="${i}">${Array.from({length:st.days},(_,n)=>`<option value="${n+1}" ${d.day===n+1?'selected':''}>${n+1}</option>`).join('')}</select><button data-edit-daytrip="${i}">${es?'Editar':'Edit'}</button></div>`).join('')}</div>`:''}`);
      active.querySelector('[data-add-daytrip]').onclick=()=>{st.dayTrips.push(_tripStoryEnsureDayTrip_({day:1,countryCode:st.countryCode,country:st.country,place:'',outbound:{},return:{}}));phase='daytrip';render();};active.querySelectorAll('[data-edit-daytrip]').forEach(b=>b.onclick=()=>{st._editingDayTrip=Number(b.dataset.editDaytrip);phase='daytrip';render();});active.querySelectorAll('[data-move-daytrip]').forEach(x=>x.onchange=()=>{st.dayTrips[Number(x.dataset.moveDaytrip)].day=Number(x.value);persist();renderStory();});active.querySelector('[data-add-next]').onclick=()=>{pendingNextStay=_tripStoryEnsureStay_({days:1});story.stays.push(pendingNextStay);activeStay=story.stays.length-1;phase='stay';render();};active.querySelector('[data-finish]').onclick=()=>{story.ended=true;phase='return';render();};return;
    }
    if(phase==='daytrip'){
