...
/* =========================================================
   ITBMO V2.7.1 · TRIP STORY / CONTINUOUS JOURNEY BUILDER
   One chronological story is the source of truth. Legacy city UI remains
   hidden and is populated only as a compatibility bridge for saved-trip,
   payment, recovery and generation infrastructure.
========================================================= */
...
function _tripStoryLocationTypeOptions_(selected='city'){const es=getLang()==='es';return [['city',es?'Ciudad / lugar':'City / place'],['airport',es?'Aeropuerto':'Airport'],['rail_station',es?'Estación de tren / metro':'Rail / metro station'],['bus_station',es?'Terminal de bus':'Bus terminal'],['port',es?'Puerto / ferry':'Port / ferry'],['other',es?'Otro punto':'Other location']].map(([v,l])=>`<option value="${v}" ${v===selected?'selected':''}>${l}</option>`).join('');}
function _tripStoryCountryOptions_(selected=''){const es=getLang()==='es';return `<option value="">${es?'Selecciona país':'Select country'}</option>`+_countryOptions_().map(x=>`<option value="${_tripStoryEsc_(x.code)}" ${x.code===selected?'selected':''}>${_tripStoryEsc_(x.label)}</option>`).join('');}
function _tripStoryTimeOptions_(selected='',allowBlank=true){const es=getLang()==='es';let out=allowBlank?`<option value="">${es?'Aún no lo sé':'Not sure yet'}</option>`:'';for(let h=0;h<24;h++)for(let m of [0,30]){const v=`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`;out+=`<option value="${v}" ${v===selected?'selected':''}>${v}</option>`;}return out;}
...
function _tripStoryEnsureStay_(st={}){st.countryCode=st.countryCode||'';st.country=st.country||'';st.place=st.place||'';st.days=Math.max(1,Number(st.days||1));st.startDate=st.startDate||'';st.transportMode=st.transportMode||'recommend';st.departureTime=st.departureTime||'';st.arrivalTime=st.arrivalTime||'';st.arrivalDate=st.arrivalDate||'';st.dayTrips=Array.isArray(st.dayTrips)?st.dayTrips:[];st.perDay=Array.from({length:st.days},(_,i)=>_tripStoryDefaultHours_(i,st));return st;}
...
function renderTripStorySummary(){const host=qs('#trip-story-summary');if(!host)return;const story=_tripStoryCurrent_(),stays=story.stays||[];if(!stays.length){host.hidden=true;return;}host.hidden=false;const es=getLang()==='es';const nodes=[];if(story.start?.origin?.label)nodes.push(`<div class="trip-story-node trip-story-node--origin"><span>✦</span><b>${_tripStoryEsc_(story.start.origin.label)}</b><small>${es?'Comienza':'Starts'} · ${_tripStoryDMY_(story.start.date)}</small></div>`);stays.forEach(x=>{if(nodes.length)nodes.push('<div class="trip-story-arrow">→</div>');nodes.push(`<div class="trip-story-node"><span>⌖</span><b>${_tripStoryEsc_(x.place)}</b><small>${_tripStoryDMY_(x.startDate)} · ${Number(x.days||1)} ${Number(x.days||1)===1?(es?'día':'day'):(es?'días':'days')}</small></div>`)});if(story.returnTrip?.enabled&&story.returnTrip?.arrival?.label){nodes.push('<div class="trip-story-arrow">→</div>',`<div class="trip-story-node"><span>⌂</span><b>${_tripStoryEsc_(story.returnTrip.arrival.label)}</b><small>${es?'Regreso':'Return'}</small></div>`)}host.innerHTML=`<div class="trip-story-summary__head"><div><small>${es?'TU HISTORIA':'YOUR STORY'}</small><strong>${es?'Tu viaje, en orden':'Your trip, in order'}</strong></div><button type="button" id="edit-trip-story">${es?'Editar viaje':'Edit trip'}</button></div><div class="trip-story-timeline">${nodes.join('')}</div>`;qs('#edit-trip-story')?.addEventListener('click',openTripStoryBuilder);}
...
function openTripStoryBuilder(){
 const es=getLang()==='es',story=JSON.parse(JSON.stringify(_tripStoryCurrent_()));story.schema_version=2;story.start=story.start||{};story.start.origin=story.start.origin||{label:'',type:'city'};story.start.arrival=story.start.arrival||{label:'',type:'city'};story.start.arrivalDate=story.start.arrivalDate||'';story.stays=(story.stays||[]).map(_tripStoryEnsureStay_);story.returnTrip=story.returnTrip||{enabled:false,origin:{},arrival:{}};
 const overlay=document.createElement('div');overlay.className='trip-story-overlay';overlay.innerHTML=`<div class="trip-story-modal trip-story-modal--premium" role="dialog" aria-modal="true"><header class="trip-story-hero"><div><span class="trip-story-kicker">✦ ${es?'LA HISTORIA DE TU VIAJE':'YOUR TRIP STORY'}</span><h3>${es?'Diseña el viaje que quieres vivir':'Design the journey you want to live'}</h3><p>${es?'De principio a fin, en una sola secuencia. ITBMO convierte tu historia en un itinerario optimizado.':'From beginning to end, in one sequence. ITBMO turns your story into an optimized itinerary.'}</p></div><button class="trip-story-close" type="button" aria-label="Close">×</button></header><div class="trip-story-progress"><span class="is-on"></span><span></span><span></span><b>${es?'Tu recorrido cobra vida mientras lo construyes':'Your journey comes alive as you build it'}</b></div><div data-story-body></div></div>`;document.body.appendChild(overlay);
 ...
 const recalc=()=>{story.stays.forEach(_tripStoryEnsureStay_);let cursor=story.start.arrivalDate||'';story.stays.forEach((st,i)=>{if(i===0){if(!st.startDate&&cursor)st.startDate=cursor;}else{const prev=story.stays[i-1];const suggested=prev.startDate?_tripStoryAddDays_(prev.startDate,prev.days):'';if(!st.startDate||st._autoDate)st.startDate=suggested;st._autoDate=true;}st.sequence=i+1;});};
 const dayCards=(st,i)=>Array.from({length:st.days},(_,d)=>{const h=st.perDay[d]||_tripStoryDefaultHours_(d,st),date=_tripStoryDayDate_(st,d),trips=(st.dayTrips||[]).filter(x=>Number(x.day)===d+1);return `<div class="trip-story-day"><div class="trip-story-day__date"><b>${es?'Día':'Day'} ${d+1}</b><span>${date?_tripStoryDMY_(date):(es?'Fecha pendiente':'Date pending')}</span></div><div class="trip-story-day__hours"><label>${es?'Empieza':'Starts'}<select data-hour="start" data-day="${d}">${_tripStoryTimeOptions_(h.start,false)}</select></label><span>→</span><label>${es?'Termina':'Ends'}<select data-hour="end" data-day="${d}">${_tripStoryTimeOptions_(h.end,false)}</select></label></div><button class="trip-story-daytour" type="button" data-add-dt="${i}" data-day="${d+1}">＋ ${es?'Excursión ida y regreso':'Same-day tour'}</button>${trips.map((dt,j)=>`<div class="trip-story-daytour-chip"><span>↗</span><input data-dt-place="${(st.dayTrips||[]).indexOf(dt)}" value="${_tripStoryEsc_(dt.place||'')}" placeholder="${es?'Ej. Versailles':'e.g. Versailles'}"><select data-dt-mode="${(st.dayTrips||[]).indexOf(dt)}">${_tripStoryTransportOptions_(dt.transportMode||'recommend')}</select><button type="button" data-remove-dt="${i}:${(st.dayTrips||[]).indexOf(dt)}">×</button></div>`).join('')}</div>`}).join('');
 const stayCard=(st,i)=>`<article class="trip-story-stay" data-stay="${i}"><div class="trip-story-stay__head"><div><span>${String(i+1).padStart(2,'0')}</span><div><small>${i===0?(es?'PRIMER DESTINO':'FIRST DESTINATION'):(es?'SIGUIENTE DESTINO':'NEXT DESTINATION')}</small><strong>${_tripStoryEsc_(st.place|| (es?'Elige tu destino':'Choose destination'))}</strong></div></div>${i?`<button type="button" class="trip-story-remove" data-remove-stay="${i}">×</button>`:''}</div>${i?`<div class="trip-story-movement"><div><span>→</span><b>${es?'Cómo llegas desde el destino anterior':'How you get here'}</b></div><select data-stay-field="transportMode">${_tripStoryTransportOptions_(st.transportMode)}</select><select data-stay-field="departureTime">${_tripStoryTimeOptions_(st.departureTime)}</select><select data-stay-field="arrivalTime">${_tripStoryTimeOptions_(st.arrivalTime)}</select></div>`:''}<div class="trip-story-destination-grid"><label>${es?'País':'Country'}<select data-stay-field="countryCode">${_tripStoryCountryOptions_(st.countryCode)}</select></label><label class="trip-story-city-search">${es?'Destino':'Destination'}<input data-stay-field="place" autocomplete="off" value="${_tripStoryEsc_(st.place)}" placeholder="${es?'Escribe 3 letras…':'Type 3 letters…'}"><div class="trip-story-suggestions" hidden></div><small>${es?'Selecciona una sugerencia o escribe el lugar manualmente.':'Choose a suggestion or enter the place manually.'}</small></label><label>${es?'¿Cuántos días?':'How many days?'}<select data-stay-field="days">${_tripStoryDaysOptions_(st.days)}</select></label><label>${es?'Primer día en este destino':'First day here'}<input type="date" data-stay-field="startDate" value="${_tripStoryEsc_(st.startDate)}" ${i?'readonly':''}></label></div><div class="trip-story-days-head"><div><b>${es?'Tus días en este destino':'Your days here'}</b><small>${es?'Ajusta únicamente las horas que ya conozcas.':'Adjust only the times you already know.'}</small></div><button type="button" data-copy-hours="${i}">${es?'Mismo horario todos los días':'Same hours every day'}</button></div><div class="trip-story-days">${dayCards(st,i)}</div></article>`;
 const render=()=>{recalc();body.innerHTML=`<section class="trip-story-section trip-story-section--start"><div class="trip-story-section-title"><span>01</span><div><small>${es?'COMIENZA TU HISTORIA':'START YOUR STORY'}</small><h4>${es?'¿Cómo comienza tu viaje?':'How does your trip begin?'}</h4></div></div><div class="trip-story-start-grid"><label>${es?'Fecha en que sales de tu origen':'Date you leave home'}<input type="date" data-start="date" value="${_tripStoryEsc_(story.start.date||'')}"></label><label>${es?'Medio principal':'Main transport'}<select data-start="transportMode">${_tripStoryTransportOptions_(story.start.transportMode||'plane')}</select></label><label>${es?'Lugar de salida':'Origin'}<input data-start="origin" value="${_tripStoryEsc_(story.start.origin?.label||'')}" placeholder="${es?'Ciudad, aeropuerto, estación…':'City, airport, station…'}"></label><label>${es?'Lugar de llegada':'Arrival point'}<input data-start="arrival" value="${_tripStoryEsc_(story.start.arrival?.label||'')}" placeholder="${es?'Ciudad, aeropuerto, estación…':'City, airport, station…'}"></label><label>${es?'Hora de salida · opcional':'Departure time · optional'}<select data-start="departureTime">${_tripStoryTimeOptions_(story.start.departureTime)}</select></label><label>${es?'Fecha de llegada · opcional':'Arrival date · optional'}<input type="date" data-start="arrivalDate" value="${_tripStoryEsc_(story.start.arrivalDate||'')}"></label><label>${es?'Hora de llegada · opcional':'Arrival time · optional'}<select data-start="arrivalTime">${_tripStoryTimeOptions_(story.start.arrivalTime)}</select></label></div><div class="trip-story-help"><span>💡</span><div><b>${es?'¿Todavía no tienes los boletos? Continúa.':'No tickets yet? Keep going.'}</b><small>${es?'Los horarios y puntos exactos son opcionales. La fecha del primer destino se define aparte: nunca asumiremos que llegas el mismo día que sales.':'Exact times and terminals are optional. Your first destination date is separate: we never assume you arrive the same day you depart.'}</small></div></div></section><section class="trip-story-section"><div class="trip-story-section-title"><span>02</span><div><small>${es?'VIVE EL RECORRIDO':'LIVE THE JOURNEY'}</small><h4>${es?'Construye tus destinos en secuencia':'Build your destinations in sequence'}</h4></div></div><div class="trip-story-stays">${story.stays.map(stayCard).join('')}</div><button class="trip-story-add trip-story-add--destination" type="button" data-add-stay><span>＋</span><div><b>${es?'Agregar destino a mi recorrido':'Add destination to my journey'}</b><small>${es?'Continúa la historia desde donde quedaste':'Continue the story from where you left off'}</small></div></button></section><section class="trip-story-section trip-story-section--finish"><div class="trip-story-section-title"><span>03</span><div><small>${es?'CIERRA EL CÍRCULO':'COMPLETE THE STORY'}</small><h4>${es?'¿Aquí termina tu viaje?':'Does your trip end here?'}</h4></div></div><label class="trip-story-return-toggle"><input type="checkbox" data-return-enabled ${story.returnTrip.enabled?'checked':''}><span></span><div><b>${es?'Agregar información de regreso':'Add return information'}</b><small>${es?'Opcional · puedes completarla después':'Optional · you can complete it later'}</small></div></label>${story.returnTrip.enabled?`<div class="trip-story-start-grid trip-story-return"><label>${es?'Fecha de salida':'Departure date'}<input type="date" data-return="departureDate" value="${_tripStoryEsc_(story.returnTrip.departureDate||'')}"></label><label>${es?'Medio':'Transport'}<select data-return="transportMode">${_tripStoryTransportOptions_(story.returnTrip.transportMode||'plane')}</select></label><label>${es?'Lugar de salida':'Origin'}<input data-return="origin" value="${_tripStoryEsc_(story.returnTrip.origin?.label||'')}"></label><label>${es?'Hora de salida':'Departure time'}<select data-return="departureTime">${_tripStoryTimeOptions_(story.returnTrip.departureTime)}</select></label><label>${es?'Lugar de llegada':'Arrival'}<input data-return="arrival" value="${_tripStoryEsc_(story.returnTrip.arrival?.label||'')}"></label><label>${es?'Fecha de llegada':'Arrival date'}<input type="date" data-return="arrivalDate" value="${_tripStoryEsc_(story.returnTrip.arrivalDate||'')}"></label><label>${es?'Hora de llegada':'Arrival time'}<select data-return="arrivalTime">${_tripStoryTimeOptions_(story.returnTrip.arrivalTime)}</select></label></div>`:''}</section><div class="trip-story-actions"><button type="button" class="secondary" data-story-cancel>${es?'Cancelar':'Cancel'}</button><button type="button" class="primary" data-story-save><span>✦</span>${es?'Guardar historia de mi viaje':'Save my trip story'} →</button></div>`;
...
function applyTripStoryToCompatibility(story){
 ...
 stays.forEach((st,i)=>{if(i>0){segments.push({id:`story_move_${i}_${Date.now()}`,origin:prev.place,destination:st.place,departureDate:st.startDate,arrivalDate:st.startDate,departureTime:st.departureTime||'',arrivalTime:st.arrivalTime||'',transportMode:st.transportMode||'recommend',timePrecision:(st.departureTime&&st.arrivalTime)?'exact':'unknown',disposition:'continue',nights:Number(st.days||1),source:'TRIP_STORY'});} (st.dayTrips||[]).forEach((dt,j)=>{if(!dt.place)return;const date=_tripStoryAddDays_(st.startDate,Math.max(0,Number(dt.day||1)-1));segments.push({id:`story_daytrip_${i}_${j}_${Date.now()}`,origin:st.place,destination:dt.place,departureDate:date,arrivalDate:date,departureTime:'',arrivalTime:'',returnDepartureDate:date,returnArrivalTime:'',returnDestination:st.place,transportMode:dt.transportMode||'recommend',returnTransportMode:dt.transportMode||'recommend',timePrecision:'unknown',disposition:'roundtrip',source:'TRIP_STORY_DAYTRIP'});});prev=st;});
