...
let _parallelTripGenerationActive = false;

function showWOW(on, msg){
  // During a multi-city parallel generation, individual city completions must
  // not close the shared trip overlay. The outer trip controller closes it once.
  if(!on && _parallelTripGenerationActive) return;
...
async function onSend(){
  ...
  if (typeof plannerState !== 'undefined' && plannerState && plannerState.collectingItineraryLang) {
    ...
    (async ()=>{
      ...
      _parallelTripGenerationActive = true;
      ...
      try{
        /* Parallel wall-clock optimization only.
           IMPORTANT: generateCityItinerary() itself is the exact known-good
           completion/render path that previously opened Focus Mode correctly. */
        await Promise.all(
          generationCities.map(city=>generateCityItinerary(city))
        );
      }finally{
        _parallelTripGenerationActive = false;
        showWOW(false);
      }

      /* Each city has already executed the original stable:
         renderCityTabs() -> setActiveCity(city) -> renderCityItinerary(city).
         Do not rebuild or replace that state here. Only refresh launcher visibility. */
      syncImmersiveItineraryLauncher();
