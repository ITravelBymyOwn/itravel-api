...
async function _tripStorySuggestions_(countryCode,query){const q=String(query||'').trim(),global=countryCode==='*',match=global?null:_tripStoryCountryFromCode_(countryCode);if((!global&&!match)||q.length<3)return[];const key=`story|${global?'GLOBAL':match.code}|${_normalizeSearch_(q)}`;if(destinationSuggestionCache.has(key))return destinationSuggestionCache.get(key);try{const url=global?`${ITBMO_DESTINATION_SUGGESTIONS_URL}?global=1&lang=${encodeURIComponent(getLang())}&q=${encodeURIComponent(q)}`:`${ITBMO_DESTINATION_SUGGESTIONS_URL}?country=${encodeURIComponent(match.apiName)}&countryCode=${encodeURIComponent(match.code)}&lang=${encodeURIComponent(getLang())}&q=${encodeURIComponent(q)}`;const r=await fetch(url,{headers:{Accept:'application/json'}}),d=await r.json().catch(()=>({}));const a=global?(r.ok&&Array.isArray(d?.results)?d.results.map(x=>x.city).filter(Boolean):[]):(r.ok&&Array.isArray(d?.suggestions)?d.suggestions:[]);destinationSuggestionCache.set(key,a);return a;}catch(_){return[];}}
