...
function _localGlobalAudit_(city,rows,totalDays,masterDays,perDay,baseDate='',routeContextOverride=undefined,expectedDaysOverride=undefined){
  ...
  for(const day of expectedAuditDays){
    ...
    for(let i=1;i<dayRows.length;i++){
      ...
      const context=`${prev.activity||''} ${prev.notes||''} ${next.activity||''} ${next.notes||''}`;
      const explained=/\b(lunch|dinner|breakfast|meal|rest|break|buffer|margin|check[- ]?in|security|boarding|transfer|station|airport|almuerzo|cena|desayuno|comida|descanso|pausa|margen|facturaci[oó]n|seguridad|embarque|traslado|estaci[oó]n|aeropuerto)\b/i.test(context);
