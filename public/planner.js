...
function openTripStoryBuilder(){
 ...
 const es=getLang()==='es',draft=_tripStoryLoadDraft_(),story=JSON.parse(JSON.stringify(draft||_tripStoryCurrent_()));story.schema_version=5;story.start=story.start||{};story.start.origin=story.start.origin||{label:'',type:'city'};story.start.arrival=story.start.arrival||{label:'',type:'city'};story.stays=(story.stays||[]).map(_tripStoryEnsureStay_);story.returnTrip=story.returnTrip||{enabled:false,origin:{},arrival:{}};let activeStay=Math.max(0,story.stays.length-1),dragStay=null,dragDayTrip=null,startExpanded=false;
 const overlay=document.createElement('div');overlay.className='trip-story-overlay';overlay.innerHTML=`<div class="trip-story-modal trip-story-modal--premium" role="dialog" aria-modal="true"><header class="trip-story-hero"><div><span class="trip-story-kicker">✦ ${es?'LA HISTORIA DE TU VIAJE':'YOUR TRIP STORY'}</span><h3>${es?'Diseña el viaje que quieres vivir':'Design the journey you want to live'}</h3><p>${es?'Construye tu viaje como una historia. Puedes mover estancias, cambiar fechas y reorganizar excursiones sin empezar de nuevo.':'Build your journey like a story. Reorder stays, change dates and move day trips without starting over.'}</p></div><button class="trip-story-close" type="button">×</button></header><div class="trip-story-progress"><span class="is-on"></span><span class="is-on"></span><span class="is-on"></span><b>${es?'Salida → estancias → excursiones → regreso':'Departure → stays → day trips → return'}</b></div><div data-story-body></div></div>`;document.body.appendChild(overlay);document.body.classList.add('trip-story-open');
 const body=overlay.querySelector('[data-story-body]'),persistDraft=()=>_tripStorySaveDraft_(story),close=()=>{persistDraft();overlay.remove();document.body.classList.remove('trip-story-open');};overlay.querySelector('.trip-story-close').onclick=close;
 const countryField=(value,code,attrs='')=>`<div class="trip-story-location-field"><input autocomplete="off" ${attrs} value="${_tripStoryEsc_(value||'')}" placeholder="${es?'Escribe 2 letras…':'Type 2 letters…'}" data-country-code="${_tripStoryEsc_(code||'')}"><div class="trip-story-suggestions" hidden></div></div>`;
 const destinationField=(value,attrs='')=>`<div class="trip-story-location-field"><input autocomplete="off" ${attrs} value="${_tripStoryEsc_(value||'')}" placeholder="${es?'Escribe 3 letras…':'Type 3 letters…'}"><div class="trip-story-suggestions" hidden></div></div>`;
 const installCopyControl=()=>{const actions=body.querySelector('.trip-story-actions__left');if(!actions||actions.querySelector('[data-story-copy]'))return;const button=document.createElement('button');button.type='button';button.className='trip-story-copy';button.dataset.storyCopy='';button.textContent=es?'Copiar recorrido':'Copy journey';button.addEventListener('click',async()=>{_tripStoryApplyBoundaryHours_(story);persistDraft();const txt=_tripStoryCopyText_(story);try{await navigator.clipboard.writeText(txt);const old=button.textContent;button.textContent=es?'✓ Copiado':'✓ Copied';setTimeout(()=>{if(button.isConnected)button.textContent=old;},1600);}catch(_){prompt(es?'Copia tu recorrido:':'Copy your journey:',txt);}});actions.insertBefore(button,actions.querySelector('[data-story-reset]'));};
 const copyControlObserver=new MutationObserver(installCopyControl);copyControlObserver.observe(body,{childList:true,subtree:true});
 const dayTripCard=(st,si,dt,di)=>`<div class="trip-story-daytrip-card" draggable="true" data-dt-drag="${si}:${di}"><div class="trip-story-daytrip-head"><div><span>↗</span><div><small>${es?'EXCURSIÓN DE UN DÍA':'DAY TRIP'}</small><strong>${_tripStoryEsc_(dt.place|| (es?'Elige dónde quieres ir':'Choose where to go'))}</strong></div></div><div class="trip-story-mini-actions"><label>${es?'Mover al':'Move to'} <select data-move-dt="${si}:${di}">${Array.from({length:st.days},(_,d)=>`<option value="${d+1}" ${dt.day===d+1?'selected':''}>${es?'Día':'Day'} ${d+1}</option>`).join('')}</select></label><button type="button" data-remove-dt="${si}:${di}">×</button></div></div><p>${es?`Sales de <b>${_tripStoryEsc_(st.place)}</b>, visitas este lugar y regresas el mismo día. Puedes mover la excursión a otro día en cualquier momento. Si no conoces transporte u horarios, déjalos en blanco y ITBMO los resolverá antes de generar.`:`Leave <b>${_tripStoryEsc_(st.place)}</b>, visit this place and return the same day. You can move the day trip to another day at any time. If transport or times are unknown, leave them blank and ITBMO will resolve them before generation.`}</p><div class="trip-story-daytrip-destination"><label><span>${es?'País':'Country'}</span><small>${es?'Escribe y selecciona una opción.':'Type and select an option.'}</small>${countryField(dt.country,dt.countryCode,`data-dt-country="${di}"`)}</label><label><span>${es?'Destino de la excursión':'Day-trip destination'}</span><small>${es?'Selecciona una sugerencia o conserva el lugar escrito.':'Choose a suggestion or keep the typed place.'}</small>${destinationField(dt.place,`data-dt-place="${di}"`)}</label></div>${[['outbound',es?'IDA':'OUTBOUND',`${st.place} → ${dt.place||'destino'}`],['return',es?'REGRESO':'RETURN',`${dt.place||'destino'} → ${st.place}`]].map(([kind,label,route])=>`<div class="trip-story-leg"><div class="trip-story-leg__title"><span>${label}</span><b>${_tripStoryEsc_(route)}</b></div><label><span>${es?'Medio de transporte':'Transport'}</span><select data-dt-${kind==='outbound'?'out':'return'}="transportMode" data-dt-index="${di}">${_tripStoryTransportOptions_(dt[kind].transportMode)}</select></label><label><span>${es?'Hora de salida':'Departure time'}</span><select data-dt-${kind==='outbound'?'out':'return'}="departureTime" data-dt-index="${di}">${_tripStoryTimeOptions_(dt[kind].departureTime)}</select></label><label><span>${es?'Hora de llegada':'Arrival time'}</span><select data-dt-${kind==='outbound'?'out':'return'}="arrivalTime" data-dt-index="${di}">${_tripStoryTimeOptions_(dt[kind].arrivalTime)}</select></label></div>`).join('')}</div>`;
 const stayCard=(st,i)=>{const collapsed=i!==activeStay;return `<article class="trip-story-stay ${collapsed?'is-collapsed':''}" data-stay="${i}" draggable="true"><div class="trip-story-stay__head"><div class="trip-story-drag" title="${es?'Arrastra para cambiar el orden':'Drag to reorder'}">⋮⋮</div><div><span>${String(i+1).padStart(2,'0')}</span><div><small>${i===0?(es?'PRIMER DESTINO':'FIRST DESTINATION'):(es?'SIGUIENTE DESTINO':'NEXT DESTINATION')}</small><strong>${_tripStoryEsc_(st.place|| (es?'Elige tu destino':'Choose destination'))}</strong><em>${st.startDate?_tripStoryDMY_(st.startDate):'—'}${_tripStoryStayEnd_(st)!==st.startDate?` → ${_tripStoryDMY_(_tripStoryStayEnd_(st))}`:''} · ${st.days} ${st.days===1?(es?'día':'day'):(es?'días':'days')}</em></div></div><div class="trip-story-head-actions">${i?`<button type="button" data-move-stay="${i}" data-dir="-1" title="${es?'Mover arriba':'Move up'}">↑</button>`:''}${i<story.stays.length-1?`<button type="button" data-move-stay="${i}" data-dir="1" title="${es?'Mover abajo':'Move down'}">↓</button>`:''}<button type="button" data-toggle-stay="${i}">${collapsed?(es?'Ampliar':'Expand'):(es?'Contraer':'Collapse')}</button>${i?`<button class="trip-story-remove" type="button" data-remove-stay="${i}">×</button>`:''}</div></div><div class="trip-story-stay__body"><div class="trip-story-card-guide"><b>${es?'Primero el lugar. Después, las fechas y cómo llegas.':'First the place. Then dates and how you get there.'}</b><span>${es?'Si cambias una fecha, ITBMO recalcula automáticamente las estancias posteriores; todas siguen siendo editables.':'If you change a date, ITBMO automatically recalculates later stays; every date remains editable.'}</span></div><div class="trip-story-destination-grid"><label><span>${es?'País':'Country'}</span><small>${es?'Escribe al menos 2 letras.':'Type at least 2 letters.'}</small>${countryField(st.country,st.countryCode,'data-stay-country')}</label><label><span>${es?'Destino':'Destination'}</span><small>${es?'Escribe 3 letras; si no aparece, conserva lo escrito.':'Type 3 letters; if not listed, keep what you typed.'}</small>${destinationField(st.place,'data-stay-place')}</label><label><span>${es?'¿Cuántos días estarás aquí?':'How many days?'}</span><small>${es?'La duración se conserva al mover la estancia.':'Duration is preserved when reordered.'}</small><select data-stay-field="days">${_tripStoryDaysOptions_(st.days)}</select></label><label><span>${es?'Primer día que quieres planificar':'First day to plan'}</span><small>${es?'Editable. Al cambiarlo, recalculamos desde aquí.':'Editable. Changes recalculate everything after it.'}</small><input type="date" data-stay-field="startDate" value="${_tripStoryEsc_(st.startDate)}"></label></div>${i?`<div class="trip-story-movement trip-story-movement--resolver"><div class="trip-story-movement__title"><span>→</span><div><small>${es?'TRASLADO AL SIGUIENTE DESTINO':'TRANSFER TO THE NEXT DESTINATION'}</small><b>${_tripStoryEsc_(story.stays[i-1]?.place||'')} → ${_tripStoryEsc_(st.place|| (es?'siguiente destino':'next destination'))}</b><p>${es?'Indica siempre el día y desde qué hora puedes salir. Si ya tienes el transporte definido, agrégalo; si no, déjalo en blanco y ITBMO analizará una ruta viable, conexiones y llegada estimada.':'Always enter the day and the earliest time you can leave. If transport is already defined, add it; otherwise leave it blank and ITBMO will analyze a viable route, connections and estimated arrival.'}</p></div></div><label><span>${es?'Día del traslado · obligatorio':'Transfer date · required'}</span><input type="date" data-stay-field="departureDate" value="${_tripStoryEsc_(st.departureDate||'')}"><small>${es?'Puede coincidir con el último día anterior o el primer día aquí.':'It may match the previous last day or this destination’s first day.'}</small></label><label><span>${es?'Puedo salir desde · obligatorio':'I can leave from · required'}</span><select data-stay-field="departureTime">${_tripStoryRequiredTimeOptions_(st.departureTime)}</select><small>${es?'Es la hora desde la que estás disponible, no necesariamente la salida de un vuelo o tren.':'This is when you become available, not necessarily a flight or train departure.'}</small></label><label><span>${es?'Medio de transporte · opcional':'Transport · optional'}</span><select data-stay-field="transportMode">${_tripStoryTransportOptions_(st.transportMode)}</select><small>${es?'Indícalo sólo si ya lo tienes definido.':'Only choose it if you already know it.'}</small></label><label><span>${es?'Fecha de llegada · opcional':'Arrival date · optional'}</span><input type="date" data-stay-field="arrivalDate" value="${_tripStoryEsc_(st.arrivalDate||'')}"><small>${es?'Si queda en blanco, ITBMO la estimará.':'Leave blank and ITBMO will estimate it.'}</small></label><label><span>${es?'Hora de llegada · opcional':'Arrival time · optional'}</span><select data-stay-field="arrivalTime">${_tripStoryTimeOptions_(st.arrivalTime)}</select><small>${es?'Si la conoces, se respetará como dato del usuario.':'If known, it will be preserved as user input.'}</small></label><div class="trip-story-resolver-note"><span>✦</span><div><b>${st.routeResolution?(es?'Ruta estimada por ITBMO':'Route estimated by ITBMO'):(es?'ITBMO completa lo que no sabes':'ITBMO fills in what you do not know')}</b><small>${st.routeResolution? _tripStoryEsc_(st.routeResolution.summary||'') : (es?'Podemos resolver trenes, aeropuertos, conexiones y tramos multimodales sin obligarte a investigarlos antes.':'We can resolve trains, airports, connections and multimodal legs without making you research them first.')}</small></div></div></div>`:''}<div class="trip-story-days-head"><div><b>${es?'Tus días en este destino':'Your days here'}</b><span>${es?'Los horarios son tu disponibilidad. Los traslados y excursiones recortan automáticamente la ventana física útil.':'These hours are your availability. Transfers and day trips automatically constrain the useful physical window.'}</span></div><button type="button" data-copy-hours>${es?'Aplicar el horario del Día 1 a todos':'Apply Day 1 hours to all'}</button></div><div class="trip-story-days">${st.perDay.map((d,di)=>`<div class="trip-story-day" data-day-drop="${i}:${di+1}"><div class="trip-story-day__main"><div class="trip-story-day__date"><b>${es?'Día':'Day'} ${di+1}</b><span>${_tripStoryDMY_(_tripStoryDayDate_(st,di))}</span></div><div class="trip-story-day__hours"><label><span>${es?'Empieza':'Starts'}</span><select data-hour="start" data-day="${di}">${_tripStoryTimeOptions_(d.start,true).replace(/<option value="(\d{2}:\d{2})"/g,(m,t)=>di===0&&st.arrivalTime&&_tripStoryTimeMinutes_(t)<_tripStoryTimeMinutes_(st.arrivalTime)?m.replace('<option ','<option disabled '):m)}</select></label><span>→</span><label><span>${es?'Termina':'Ends'}</span><select data-hour="end" data-day="${di}">${_tripStoryTimeOptions_(d.end,true).replace(/<option value="(\d{2}:\d{2})"/g,(m,t)=>di===st.perDay.length-1&&((story.stays[i+1]?.departureTime&&_tripStoryTimeMinutes_(t)>_tripStoryTimeMinutes_(story.stays[i+1].departureTime))||(!story.stays[i+1]&&story.returnTrip?.enabled&&story.returnTrip?.departureTime&&_tripStoryTimeMinutes_(t)>_tripStoryTimeMinutes_(story.returnTrip.departureTime)))?m.replace('<option ','<option disabled '):m)}</select></label></div><button type="button" class="trip-story-daytour" data-add-dt="${i}" data-day="${di+1}">＋ ${es?'Agregar excursión de un día':'Add day trip'}</button></div>${st.dayTrips.filter(x=>Number(x.day)===di+1).map(dt=>dayTripCard(st,i,dt,st.dayTrips.indexOf(dt))).join('')}</div>`).join('')}</div></div></article>`;};
 const render=()=>{story.stays.forEach(_tripStoryEnsureStay_);_tripStoryApplyBoundaryHours_(story);persistDraft();body.innerHTML=`<section class="trip-story-section trip-story-section--start ${startExpanded?'is-expanded':'is-collapsed'}"><div class="trip-story-section-title trip-story-section-title--toggle"><span>01</span><div><small>${es?'COMIENZA TU HISTORIA':'START YOUR STORY'}</small><h4>${es?'¿Cómo comienza tu viaje?':'How does your journey begin?'}</h4><p>${es?'Opcional. Agrega únicamente lo que ya conozcas de tu llegada al primer destino.':'Optional. Add only what you already know about reaching your first destination.'}</p></div><button type="button" class="trip-story-section-toggle" data-toggle-start>${startExpanded?(es?'Contraer':'Collapse'):(es?'Agregar información de inicio':'Add start information')}</button></div>${startExpanded?`<div class="trip-story-start-grid trip-story-start-grid--wide"><label><span>${es?'Fecha en que sales de tu origen · opcional':'Origin departure date · optional'}</span><input type="date" data-start="date" value="${_tripStoryEsc_(story.start.date||'')}"></label><label><span>${es?'Medio principal':'Main transport'}</span><select data-start="transportMode">${_tripStoryTransportOptions_(story.start.transportMode||'plane')}</select></label><label><span>${es?'Lugar de salida · opcional':'Departure place · optional'}</span><input data-start="origin" value="${_tripStoryEsc_(story.start.origin?.label||'')}" placeholder="Ej. San José / SJO"></label><label><span>${es?'Hora de salida · opcional':'Departure time · optional'}</span><select data-start="departureTime">${_tripStoryTimeOptions_(story.start.departureTime)}</select></label><label><span>${es?'Lugar de llegada · opcional':'Arrival place · optional'}</span><input data-start="arrival" value="${_tripStoryEsc_(story.start.arrival?.label||'')}" placeholder="Ej. Madrid / MAD"></label><label><span>${es?'Fecha de llegada · opcional':'Arrival date · optional'}</span><input type="date" data-start="arrivalDate" value="${_tripStoryEsc_(story.start.arrivalDate||'')}"></label><label><span>${es?'Hora de llegada · opcional':'Arrival time · optional'}</span><select data-start="arrivalTime">${_tripStoryTimeOptions_(story.start.arrivalTime)}</select></label></div>`:`<button type="button" class="trip-story-optional-card" data-toggle-start><span>＋</span><div><b>${(story.start?.date||story.start?.origin?.label||story.start?.arrival?.label||story.start?.departureTime||story.start?.arrivalDate||story.start?.arrivalTime)?(es?'✓ Información de inicio agregada · Editar':'✓ Start information added · Edit'):(es?'Agregar información de inicio':'Add start information')}</b><small>${es?'Opcional · vuelo, tren o traslado con el que llegas al primer destino.':'Optional · flight, train or transfer used to reach your first destination.'}</small></div></button>`}</section><section class="trip-story-section"><div class="trip-story-section-title"><span>02</span><div><small>${es?'VIVE EL RECORRIDO':'BUILD THE JOURNEY'}</small><h4>${es?'Ordena tu viaje, estancia por estancia':'Arrange your journey, stay by stay'}</h4><p>${es?'Arrastra una estancia para cambiar el orden. ITBMO conserva su duración y sus excursiones y recalcula las fechas posteriores.':'Drag a stay to change the order. ITBMO preserves duration and day trips and recalculates later dates.'}</p></div></div><div class="trip-story-stays">${story.stays.map(stayCard).join('')}</div><button class="trip-story-add trip-story-add--destination" type="button" data-add-stay><span>＋</span><div><b>${es?'Agregar siguiente destino':'Add next destination'}</b><small>${es?'También puedes reordenarlo después sin volver a escribirlo.':'You can reorder it later without retyping it.'}</small></div></button></section><section class="trip-story-section trip-story-section--finish"><div class="trip-story-section-title"><span>03</span><div><small>${es?'CIERRA TU HISTORIA':'FINISH YOUR STORY'}</small><h4>${es?'¿Aquí termina tu recorrido?':'Does your journey end here?'}</h4><p>${es?'Confírmalo para que ITBMO sepa que no olvidaste agregar otro destino. La información del regreso sigue siendo opcional.':'Confirm it so ITBMO knows you did not forget another destination. Return information remains optional.'}</p></div></div><label class="trip-story-end-confirm"><input type="checkbox" data-story-ended ${story.ended?'checked':''}><span>✓</span><div><b>${es?'Sí, aquí termina mi recorrido':'Yes, my journey ends here'}</b><small>${es?'Podrás editarlo incluso después de guardarlo.':'You can still edit it after saving.'}</small></div></label><label class="trip-story-return-toggle"><input type="checkbox" data-return-enabled ${story.returnTrip.enabled?'checked':''}><span></span><div><b>${es?'Agregar información de regreso':'Add return information'}</b><small>${es?'Opcional · completa solo lo que ya conozcas':'Optional · complete only what you know'}</small></div></label>${story.returnTrip.enabled?`<div class="trip-story-start-grid trip-story-return"><label><span>${es?'Fecha de salida':'Departure date'}</span><input type="date" data-return="departureDate" value="${_tripStoryEsc_(story.returnTrip.departureDate||'')}"></label><label><span>${es?'Medio de transporte':'Transport'}</span><select data-return="transportMode">${_tripStoryTransportOptions_(story.returnTrip.transportMode||'plane')}</select></label><label><span>${es?'Lugar de salida':'Origin'}</span><input data-return="origin" value="${_tripStoryEsc_(story.returnTrip.origin?.label||'')}"></label><label><span>${es?'Hora de salida · opcional':'Departure time · optional'}</span><select data-return="departureTime">${_tripStoryTimeOptions_(story.returnTrip.departureTime)}</select></label><label><span>${es?'Lugar de llegada':'Arrival'}</span><input data-return="arrival" value="${_tripStoryEsc_(story.returnTrip.arrival?.label||'')}"></label><label><span>${es?'Fecha de llegada · opcional':'Arrival date · optional'}</span><input type="date" data-return="arrivalDate" value="${_tripStoryEsc_(story.returnTrip.arrivalDate||'')}"></label><label><span>${es?'Hora de llegada · opcional':'Arrival time · optional'}</span><select data-return="arrivalTime">${_tripStoryTimeOptions_(story.returnTrip.arrivalTime)}</select></label></div>`:''}</section><div class="trip-story-actions"><div class="trip-story-actions__left"><button type="button" class="secondary" data-story-cancel>${es?'Cerrar':'Close'}</button><button type="button" class="trip-story-reset" data-story-reset>${es?'Reiniciar recorrido':'Reset journey'}</button></div><div class="trip-story-actions__save"><small>${story.ended?(es?'✓ Recorrido listo para guardar':'✓ Journey ready to save'):(es?'Antes de guardar, confirma dónde termina':'Before saving, confirm where it ends')}</small><button type="button" class="primary" data-story-save><span>✦</span>${es?'Guardar historia de mi viaje':'Save my trip story'} →</button></div></div>`;bind();};
 const bindLocation=(input,onPick,kind='country',countryCode=()=>'' )=>{if(!input)return;let timer;input.oninput=()=>{onPick({label:input.value,code:'',typing:true});clearTimeout(timer);const menu=input.parentElement.querySelector('.trip-story-suggestions');const q=input.value.trim();if(q.length<(kind==='country'?2:3)){menu.hidden=true;return;}if(kind==='country'){const items=_tripStoryCountrySuggestions_(q);menu.innerHTML=items.length?items.map(x=>`<button type="button">${_tripStoryEsc_(x.label)}</button>`).join(''):`<div>${es?'Sigue escribiendo para encontrar el país.':'Keep typing to find the country.'}</div>`;menu.hidden=false;menu.querySelectorAll('button').forEach((b,n)=>b.onclick=()=>{onPick({label:items[n].label,code:items[n].code});render();});}else{timer=setTimeout(async()=>{const items=await _tripStorySuggestions_(countryCode(),q);if(input.value.trim()!==q)return;menu.innerHTML=items.length?items.slice(0,10).map(x=>`<button type="button">${_tripStoryEsc_(typeof x==='string'?x:x.label)}</button>`).join(''):`<div>${es?'No aparece en la lista. Puedes conservar lo escrito.':'Not listed. You can keep what you typed.'}</div>`;menu.hidden=false;menu.querySelectorAll('button').forEach((b,n)=>b.onclick=()=>{onPick({label:typeof items[n]==='string'?items[n]:items[n].label});render();});},180);}};};
 const bind=()=>{body.querySelectorAll('[data-toggle-start]').forEach(b=>b.onclick=()=>{startExpanded=!startExpanded;render();});body.querySelectorAll('[data-start]').forEach(el=>el.onchange=()=>{const k=el.dataset.start;if(k==='origin')story.start.origin.label=el.value;else if(k==='arrival')story.start.arrival.label=el.value;else story.start[k]=el.value;persistDraft();});body.querySelectorAll('[data-toggle-stay]').forEach(b=>b.onclick=()=>{const target=Number(b.dataset.toggleStay);activeStay=activeStay===target?-1:target;render();if(activeStay>=0)setTimeout(()=>body.querySelector(`[data-stay="${activeStay}"]`)?.scrollIntoView({behavior:'smooth',block:'start'}),60);});body.querySelectorAll('[data-stay]').forEach(card=>{const i=Number(card.dataset.stay),st=story.stays[i];bindLocation(card.querySelector('[data-stay-country]'),x=>{st.country=x.label;if(x.code)st.countryCode=x.code;persistDraft();},'country');bindLocation(card.querySelector('[data-stay-place]'),x=>{st.place=x.label;persistDraft();},'city',()=>st.countryCode);card.querySelectorAll('[data-stay-field]').forEach(el=>el.onchange=()=>{const k=el.dataset.stayField;if(k==='days'){st.days=Number(el.value);_tripStoryEnsureStay_(st);_tripStoryRecalculateFrom_(story,i+1,{preserveAnchor:false});render();return;}st[k]=el.value;if(k==='startDate'){st.departureDate=el.value;st.arrivalDate=el.value;_tripStoryRecalculateFrom_(story,i+1,{preserveAnchor:false});render();return;}if(k==='arrivalTime'){if(!st.arrivalDate||st.arrivalDate===st.startDate)_tripStoryClampArrivalStart_(st);render();return;}if(k==='departureTime'&&i>0){if(st.departureDate===_tripStoryStayEnd_(story.stays[i-1]))_tripStoryClampDepartureEnd_(story.stays[i-1],st.departureTime);render();return;}persistDraft();});card.querySelectorAll('[data-hour]').forEach(el=>el.onchange=()=>{st.perDay[Number(el.dataset.day)][el.dataset.hour]=el.value;persistDraft();});card.querySelector('[data-copy-hours]')?.addEventListener('click',()=>{const f=st.perDay[0];st.perDay=Array.from({length:st.days},(_,d)=>({day:d+1,start:f.start,end:f.end}));render();});card.querySelectorAll('[data-dt-country]').forEach(el=>{const dt=st.dayTrips[Number(el.dataset.dtCountry)];bindLocation(el,x=>{dt.country=x.label;if(x.code)dt.countryCode=x.code;persistDraft();},'country');});card.querySelectorAll('[data-dt-place]').forEach(el=>{const dt=st.dayTrips[Number(el.dataset.dtPlace)];bindLocation(el,x=>{dt.place=x.label;persistDraft();},'city',()=>dt.countryCode);});card.querySelectorAll('[data-dt-out]').forEach(el=>el.onchange=()=>{st.dayTrips[Number(el.dataset.dtIndex)].outbound[el.dataset.dtOut]=el.value;persistDraft();});card.querySelectorAll('[data-dt-return]').forEach(el=>el.onchange=()=>{st.dayTrips[Number(el.dataset.dtIndex)].return[el.dataset.dtReturn]=el.value;persistDraft();});});body.querySelectorAll('[data-move-stay]').forEach(b=>b.onclick=()=>{const from=Number(b.dataset.moveStay),to=from+Number(b.dataset.dir);if(to<0||to>=story.stays.length)return;const anchor=story.stays[0]?.startDate,[moved]=story.stays.splice(from,1);story.stays.splice(to,0,moved);if(anchor)story.stays[0].startDate=anchor;activeStay=to;_tripStoryRecalculateFrom_(story,1,{preserveAnchor:false});render();});body.querySelectorAll('[data-remove-stay]').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.removeStay);story.stays.splice(i,1);activeStay=Math.max(0,Math.min(activeStay,story.stays.length-1));_tripStoryRecalculateFrom_(story,Math.max(1,i),{preserveAnchor:false});render();});body.querySelector('[data-add-stay]')?.addEventListener('click',()=>{const prev=story.stays.at(-1),start=prev?.startDate?_tripStoryStayEnd_(prev):'';story.stays.push(_tripStoryEnsureStay_({startDate:start,arrivalDate:'',departureDate:start,days:1,transportMode:''}));activeStay=story.stays.length-1;render();setTimeout(()=>{const card=body.querySelector(`[data-stay="${activeStay}"]`);card?.scrollIntoView({behavior:'smooth',block:'start'});card?.querySelector('[data-stay-country]')?.focus();},80);});body.querySelectorAll('[data-add-dt]').forEach(b=>b.onclick=()=>{const st=story.stays[Number(b.dataset.addDt)];st.dayTrips.push(_tripStoryEnsureDayTrip_({day:Number(b.dataset.day),countryCode:st.countryCode,country:st.country,place:'',outbound:{transportMode:''},return:{transportMode:''}}));activeStay=Number(b.dataset.addDt);render();});body.querySelectorAll('[data-remove-dt]').forEach(b=>b.onclick=()=>{const [i,j]=b.dataset.removeDt.split(':').map(Number);story.stays[i].dayTrips.splice(j,1);render();});body.querySelectorAll('[data-move-dt]').forEach(s=>s.onchange=()=>{const [i,j]=s.dataset.moveDt.split(':').map(Number);story.stays[i].dayTrips[j].day=Number(s.value);render();});body.querySelectorAll('[data-dt-drag]').forEach(card=>{card.ondragstart=e=>{e.stopPropagation();dragDayTrip=card.dataset.dtDrag;card.classList.add('is-dragging');};card.ondragend=()=>{card.classList.remove('is-dragging');dragDayTrip=null;};});body.querySelectorAll('[data-day-drop]').forEach(day=>{day.ondragover=e=>{if(!dragDayTrip)return;e.preventDefault();day.classList.add('is-day-drop-target');};day.ondragleave=()=>day.classList.remove('is-day-drop-target');day.ondrop=e=>{if(!dragDayTrip)return;e.preventDefault();e.stopPropagation();const [si,di]=dragDayTrip.split(':').map(Number),[targetStay,targetDay]=day.dataset.dayDrop.split(':').map(Number);day.classList.remove('is-day-drop-target');if(si!==targetStay)return;story.stays[si].dayTrips[di].day=targetDay;dragDayTrip=null;render();};});body.querySelectorAll('.trip-story-stay').forEach(card=>{card.ondragstart=e=>{if(e.target.closest('.trip-story-daytrip-card'))return;dragStay=Number(card.dataset.stay);card.classList.add('is-dragging');};card.ondragend=()=>{card.classList.remove('is-dragging');dragStay=null;};card.ondragover=e=>{if(dragStay===null)return;e.preventDefault();card.classList.add('is-drop-target');};card.ondragleave=()=>card.classList.remove('is-drop-target');card.ondrop=e=>{if(dragStay===null)return;e.preventDefault();const to=Number(card.dataset.stay);card.classList.remove('is-drop-target');if(to===dragStay)return;const anchor=story.stays[0]?.startDate,[moved]=story.stays.splice(dragStay,1);story.stays.splice(to,0,moved);if(anchor)story.stays[0].startDate=anchor;activeStay=to;_tripStoryRecalculateFrom_(story,1,{preserveAnchor:false});render();};});body.querySelector('[data-story-ended]').onchange=e=>{story.ended=e.target.checked;persistDraft();render();};body.querySelector('[data-return-enabled]').onchange=e=>{story.returnTrip.enabled=e.target.checked;render();};body.querySelectorAll('[data-return]').forEach(el=>el.onchange=()=>{const k=el.dataset.return;if(k==='origin')story.returnTrip.origin={...(story.returnTrip.origin||{}),label:el.value};else if(k==='arrival')story.returnTrip.arrival={...(story.returnTrip.arrival||{}),label:el.value};else story.returnTrip[k]=el.value;persistDraft();});body.querySelector('[data-story-reset]')?.addEventListener('click',async()=>{const ok=await showPlannerDecision({title:es?'¿Reiniciar todo el recorrido?':'Reset the entire journey?',message:es?'Se borrarán destinos, fechas, horarios, traslados, excursiones y la información opcional de inicio y regreso.':'Destinations, dates, times, transfers, day trips and optional start/return information will be cleared.',cancelLabel:es?'Conservar mi recorrido':'Keep my journey',confirmLabel:es?'Sí, reiniciar':'Yes, reset'});if(!ok)return;_tripStoryClearDraft_();_travelV2()?.setTripStory?.(null);if($cityList)$cityList.innerHTML='';savedDestinations=[];if(plannerState)plannerState.destinations=[];renderTripStorySummary();story.start={date:'',transportMode:'plane',origin:{label:'',type:'city'},arrival:{label:'',type:'city'},departureTime:'',arrivalDate:'',arrivalTime:''};story.stays=[_tripStoryEnsureStay_({days:1})];story.returnTrip={enabled:false,transportMode:'plane',origin:{label:'',type:'city'},arrival:{label:'',type:'city'},departureDate:'',departureTime:'',arrivalDate:'',arrivalTime:''};story.ended=false;activeStay=0;startExpanded=false;render();});body.querySelector('[data-story-cancel]').onclick=close;body.querySelector('[data-story-save]').onclick=async()=>{_tripStoryApplyBoundaryHours_(story);story.stays.forEach((st,i)=>{if(i>0){if(!st.departureDate)st.departureDate=st.startDate||'';if(st.arrivalTime)_tripStoryClampArrivalStart_(st);_tripStoryClampDepartureEnd_(story.stays[i-1],st.departureTime);}});const issues=_tripStoryValidate_(story);if(issues.length){_tripStoryShowIssues_(issues,issue=>{if(issue.stayIndex!=null){activeStay=issue.stayIndex;render();setTimeout(()=>body.querySelector(`[data-stay="${issue.stayIndex}"]`)?.scrollIntoView({behavior:'smooth',block:'center'}),50);return;}if(issue.code==='NOT_ENDED'){setTimeout(()=>{const endControl=body.querySelector('[data-story-ended]');endControl?.closest('.trip-story-section--finish')?.scrollIntoView({behavior:'smooth',block:'center'});endControl?.focus({preventScroll:true});},50);}});return;}const effectiveDates=new Set();story.stays.forEach(st=>{for(let d=0;d<Math.max(1,Number(st.days||1));d++){const date=_tripStoryAddDays_(st.startDate,d);if(date)effectiveDates.add(date);}});const effectiveDays=effectiveDates.size;if(effectiveDays>MAX_TRIP_STORY_DAYS){_tripStoryShowIssues_([{title:es?'Duración del viaje':'Trip length',message:es?`Esta versión admite hasta ${MAX_TRIP_STORY_DAYS} días efectivos de itinerario. Los días que sean únicamente de traslado no necesitan convertirse en días turísticos.`:`This version supports up to ${MAX_TRIP_STORY_DAYS} effective itinerary days. Pure transfer-only dates do not need to become sightseeing days.`}],()=>{});return;}_travelV2()?.setTripStory?.(JSON.parse(JSON.stringify(story)));applyTripStoryToCompatibility(story);renderTripStorySummary();persistDraft();overlay.remove();document.body.classList.remove('trip-story-open');const saved=await saveDestinations({showReadyModal:true,fromTripStory:true});if(saved===true)_tripStoryClearDraft_();};};
