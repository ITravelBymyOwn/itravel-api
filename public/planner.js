...
async function saveDestinations({showReadyModal=true}={}){
  ...
  if(!saveLockWarningAccepted){
    ...
    const confirmed=await showPlannerDecision({
      ...
      message:es
        ? 'Al guardar, bloquearemos viajeros, recorrido, fechas y horarios para proteger la coherencia del viaje. Todavía podrás usar Reiniciar planificación y comenzar de nuevo sin pagar mientras no hayas realizado el pago.'
...
async function exportItineraryToPDF(){
  ...
  function pageHeader(city, dayNum){
    const left = 40;
    const routeCtx=_routeV2ContextForCity_(city);
    const dayCtx=(routeCtx?.day_contexts||[]).find(d=>Number(d.day)===Number(dayNum));
    const transfers=(dayCtx?.fixed_transfers||[]);
    let physicalLabel='';
    if(transfers.length){
      const chain=[dayCtx?.start_location||transfers[0]?.origin,...transfers.map(x=>x.destination)].filter(Boolean);
      physicalLabel=[...new Set(chain)].join(' → ');
    }else{
      physicalLabel=dayCtx?.overnight_base||dayCtx?.start_location||'';
    }

    doc.setFontSize(14);
    const startPlace=dayCtx?.start_location||city;
    doc.text(String(normalizeCellText(startPlace || city || 'Itinerary')), left, 42);
    ...
    doc.text(normalizeCellText(dayLine), left, 61);

    doc.setFontSize(8.5);
    doc.text(`${yyyy}-${mm}-${dd}`, left, 80);
  }
  ...
  cities.forEach(city=>{
    ...
    days.forEach(dayNum=>{
      ...
      const routeCtx=_routeV2ContextForCity_(city);
      const dayCtx=(routeCtx?.day_contexts||[]).find(d=>Number(d.day)===Number(dayNum));
      const fixedTransfers=(dayCtx?.fixed_transfers||[]);
      const body=[];
      rows.forEach((r,rowIndex)=>{
        body.push([
          normalizeCellText(r.start), normalizeCellText(r.end), normalizeCellText(r.activity),
          normalizeCellText(r.from), normalizeCellText(r.to), normalizeCellText(r.transport),
          normalizeCellText(r.duration), normalizeCellText(r.notes)
        ]);
        const transfer=fixedTransfers.find(t=>
          String(t.departure||'')===String(r.start||'') && String(t.arrival||'')===String(r.end||'') &&
          _arePoiAliases_(r.from,t.origin) && _arePoiAliases_(r.to,t.destination)
        );
        const hasRowsAfter=rowIndex<rows.length-1;
        if(transfer && hasRowsAfter && !transfer.terminal_arrival){
          body.push([{
            content:normalizeCellText(`${String(transfer.destination||'').toUpperCase()} · ${getLang()==='es'?'Desde':'From'} ${transfer.arrival}`),
            colSpan:8,
            styles:{fontStyle:'bold',fontSize:10,cellPadding:{top:7,right:4,bottom:7,left:4},halign:'left'}
          }]);
        }
      });
...
/* =========================================================
   ITBMO V2.7.2 · TRIP STORY / CONTINUOUS JOURNEY BUILDER
   Premium narrative capture. Trip Story is the source of truth; the legacy
   destination row remains a hidden compatibility bridge for persistence,
   payment, recovery and the current generation pipeline.
========================================================= */
...
function _tripStoryCurrent_(){return _travelV2()?.state?.tripStory || {schema_version:3,start:{date:'',transportMode:'plane',origin:{label:'',type:'city'},arrival:{label:'',type:'city'},departureTime:'',arrivalDate:'',arrivalTime:''},stays:[],returnTrip:{enabled:false,transportMode:'plane',origin:{label:'',type:'city'},arrival:{label:'',type:'city'},departureDate:'',departureTime:'',arrivalDate:'',arrivalTime:''},ended:false};}
function _tripStoryDefaultHours_(dayIndex,st){return {day:dayIndex+1,start:(st?.perDay?.[dayIndex]?.start||DEFAULT_START||'09:00'),end:(st?.perDay?.[dayIndex]?.end||DEFAULT_END||'20:00')};}
function _tripStoryEnsureDayTrip_(dt={}){dt.day=Math.max(1,Number(dt.day||1));dt.countryCode=dt.countryCode||'';dt.country=dt.country||'';dt.place=dt.place||'';dt.outbound=dt.outbound||{};dt.return=dt.return||{};dt.outbound.transportMode=dt.outbound.transportMode||dt.transportMode||'recommend';dt.outbound.departureTime=dt.outbound.departureTime||dt.departureTime||'';dt.outbound.arrivalTime=dt.outbound.arrivalTime||dt.arrivalTime||'';dt.return.transportMode=dt.return.transportMode||dt.returnTransportMode||dt.outbound.transportMode||'recommend';dt.return.departureTime=dt.return.departureTime||dt.returnDepartureTime||'';dt.return.arrivalTime=dt.return.arrivalTime||dt.returnArrivalTime||'';return dt;}
function _tripStoryEnsureStay_(st={}){st.countryCode=st.countryCode||'';st.country=st.country||'';st.place=st.place||'';st.days=Math.max(1,Number(st.days||1));st.startDate=st.startDate||'';st.transportMode=st.transportMode||'recommend';st.departureTime=st.departureTime||'';st.arrivalTime=st.arrivalTime||'';st.arrivalDate=st.arrivalDate||'';st.dayTrips=(Array.isArray(st.dayTrips)?st.dayTrips:[]).map(_tripStoryEnsureDayTrip_);st.perDay=Array.from({length:st.days},(_,i)=>_tripStoryDefaultHours_(i,st));return st;}
...
function renderTripStorySummary(){const host=qs('#trip-story-summary');if(!host)return;const story=_tripStoryCurrent_(),stays=story.stays||[];if(!stays.length){host.hidden=true;return;}host.hidden=false;const es=getLang()==='es',nodes=[];if(story.start?.origin?.label)nodes.push(`<div class="trip-story-node trip-story-node--origin"><span>✦</span><b>${_tripStoryEsc_(story.start.origin.label)}</b><small>${es?'Comienza':'Starts'} · ${_tripStoryDMY_(story.start.date)}</small></div>`);stays.forEach(x=>{if(nodes.length)nodes.push('<div class="trip-story-arrow">→</div>');nodes.push(`<div class="trip-story-node"><span>⌖</span><b>${_tripStoryEsc_(x.place)}</b><small>${_tripStoryDMY_(x.startDate)} · ${Number(x.days||1)} ${Number(x.days||1)===1?(es?'día':'day'):(es?'días':'days')}</small></div>`)});if(story.returnTrip?.enabled&&story.returnTrip?.arrival?.label){nodes.push('<div class="trip-story-arrow">→</div>',`<div class="trip-story-node"><span>⌂</span><b>${_tripStoryEsc_(story.returnTrip.arrival.label)}</b><small>${es?'Regreso':'Return'}</small></div>`)}host.innerHTML=`<div class="trip-story-summary__head"><div><small>${es?'TU HISTORIA':'YOUR STORY'}</small><strong>${es?'Tu viaje, en orden':'Your trip, in order'}</strong></div><button type="button" id="edit-trip-story">${es?'Editar mi viaje':'Edit my trip'}</button></div><div class="trip-story-timeline">${nodes.join('')}</div>`;qs('#edit-trip-story')?.addEventListener('click',openTripStoryBuilder);}
...
function openTripStoryBuilder(){
 const es=getLang()==='es',story=JSON.parse(JSON.stringify(_tripStoryCurrent_()));story.schema_version=3;story.start=story.start||{};story.start.origin=story.start.origin||{label:'',type:'city'};story.start.arrival=story.start.arrival||{label:'',type:'city'};story.start.arrivalDate=story.start.arrivalDate||'';story.stays=(story.stays||[]).map(_tripStoryEnsureStay_);story.returnTrip=story.returnTrip||{enabled:false,origin:{},arrival:{}};
 const overlay=document.createElement('div');overlay.className='trip-story-overlay';overlay.innerHTML=`<div class="trip-story-modal trip-story-modal--premium" role="dialog" aria-modal="true"><header class="trip-story-hero"><div><span class="trip-story-kicker">✦ ${es?'LA HISTORIA DE TU VIAJE':'YOUR TRIP STORY'}</span><h3>${es?'Diseña el viaje que quieres vivir':'Design the journey you want to live'}</h3><p>${es?'Cuéntanos la secuencia real de tu viaje. Nosotros convertiremos cada estancia, traslado y excursión en un itinerario continuo.':'Tell us the real sequence of your journey. We will turn every stay, transfer and day trip into one continuous itinerary.'}</p></div><button class="trip-story-close" type="button" aria-label="Close">×</button></header><div class="trip-story-progress"><span class="is-on"></span><span class="is-on"></span><span></span><b>${es?'Salida → estancias → excursiones → regreso':'Departure → stays → day trips → return'}</b></div><div data-story-body></div></div>`;document.body.appendChild(overlay);
 const body=overlay.querySelector('[data-story-body]'),close=()=>overlay.remove();overlay.querySelector('.trip-story-close').onclick=close;
 const recalc=()=>{story.stays.forEach(_tripStoryEnsureStay_);story.stays.forEach((st,i)=>{if(i===0){if(!st.startDate&&story.start.arrivalDate)st.startDate=story.start.arrivalDate;}else{const prev=story.stays[i-1],suggested=prev.startDate?_tripStoryAddDays_(prev.startDate,prev.days):'';if(!st.startDate||st._autoDate)st.startDate=suggested;st._autoDate=true;}st.sequence=i+1;});};
 const countryField=(value,code,attrs='')=>`<div class="trip-story-location-field trip-story-country-search"><input autocomplete="off" ${attrs} value="${_tripStoryEsc_(value||'')}" placeholder="${es?'Escribe 2 letras…':'Type 2 letters…'}" data-country-code="${_tripStoryEsc_(code||'')}"><div class="trip-story-suggestions" hidden></div></div>`;
 const destinationField=(value,attrs='')=>`<div class="trip-story-location-field trip-story-city-search"><input autocomplete="off" ${attrs} value="${_tripStoryEsc_(value||'')}" placeholder="${es?'Escribe 3 letras…':'Type 3 letters…'}"><div class="trip-story-suggestions" hidden></div></div>`;
 const dayTripCard=(st,stayIndex,dt,dtIndex)=>`<div class="trip-story-daytrip-card" data-dt-card="${dtIndex}"><div class="trip-story-daytrip-head"><div><span>↗</span><div><small>${es?'EXCURSIÓN DE UN DÍA':'DAY TRIP'}</small><strong>${dt.place?_tripStoryEsc_(dt.place):(es?'Elige dónde quieres ir':'Choose where you want to go')}</strong></div></div><button type="button" data-remove-dt="${stayIndex}:${dtIndex}">×</button></div><p>${es?`Saldrás de <b>${_tripStoryEsc_(st.place||'tu destino')}</b>, visitarás este lugar y regresarás el mismo día. No crea una nueva estancia.`:`You will leave <b>${_tripStoryEsc_(st.place||'your destination')}</b>, visit this place and return the same day. This does not create a new stay.`}</p><div class="trip-story-daytrip-destination"><label><span>${es?'País':'Country'}</span><small>${es?'Escribe y selecciona una opción.':'Type and choose an option.'}</small>${countryField(dt.country||_tripStoryCountryFromCode_(dt.countryCode)?.label||'',dt.countryCode,`data-dt-country="${dtIndex}"`)}</label><label><span>${es?'Destino de la excursión':'Day-trip destination'}</span><small>${es?'Selecciona una sugerencia o conserva el lugar escrito.':'Choose a suggestion or keep the place you typed.'}</small>${destinationField(dt.place,`data-dt-place="${dtIndex}"`)}</label></div><div class="trip-story-leg"><div class="trip-story-leg__title"><span>IDA</span><b>${_tripStoryEsc_(st.place||'Origen')} → ${_tripStoryEsc_(dt.place|| (es?'destino':'destination'))}</b></div><label><span>${es?'Medio de transporte':'Transport'}</span><select data-dt-out="transportMode" data-dt-index="${dtIndex}">${_tripStoryTransportOptions_(dt.outbound.transportMode)}</select></label><label><span>${es?'Hora de salida':'Departure time'}</span><select data-dt-out="departureTime" data-dt-index="${dtIndex}">${_tripStoryTimeOptions_(dt.outbound.departureTime)}</select></label><label><span>${es?'Hora de llegada':'Arrival time'}</span><select data-dt-out="arrivalTime" data-dt-index="${dtIndex}">${_tripStoryTimeOptions_(dt.outbound.arrivalTime)}</select></label></div><div class="trip-story-leg trip-story-leg--return"><div class="trip-story-leg__title"><span>${es?'REGRESO':'RETURN'}</span><b>${_tripStoryEsc_(dt.place|| (es?'Destino':'Destination'))} → ${_tripStoryEsc_(st.place||'Base')}</b></div><label><span>${es?'Medio de transporte':'Transport'}</span><select data-dt-return="transportMode" data-dt-index="${dtIndex}">${_tripStoryTransportOptions_(dt.return.transportMode)}</select></label><label><span>${es?'Hora de salida':'Departure time'}</span><select data-dt-return="departureTime" data-dt-index="${dtIndex}">${_tripStoryTimeOptions_(dt.return.departureTime)}</select></label><label><span>${es?'Hora de llegada':'Arrival time'}</span><select data-dt-return="arrivalTime" data-dt-index="${dtIndex}">${_tripStoryTimeOptions_(dt.return.arrivalTime)}</select></label></div></div>`;
 const dayCards=(st,i)=>Array.from({length:st.days},(_,d)=>{const h=st.perDay[d]||_tripStoryDefaultHours_(d,st),date=_tripStoryDayDate_(st,d),trips=(st.dayTrips||[]).map((x,idx)=>({x,idx})).filter(o=>Number(o.x.day)===d+1);return `<div class="trip-story-day"><div class="trip-story-day__main"><div class="trip-story-day__date"><b>${es?'Día':'Day'} ${d+1}</b><span>${date?_tripStoryDMY_(date):(es?'Fecha pendiente':'Date pending')}</span></div><div class="trip-story-day__hours"><label><span>${es?'Empieza':'Starts'}</span><select data-hour="start" data-day="${d}">${_tripStoryTimeOptions_(h.start,false)}</select></label><span>→</span><label><span>${es?'Termina':'Ends'}</span><select data-hour="end" data-day="${d}">${_tripStoryTimeOptions_(h.end,false)}</select></label></div><button class="trip-story-daytour" type="button" data-add-dt="${i}" data-day="${d+1}">＋ ${es?'Agregar excursión de un día':'Add a day trip'}</button></div>${trips.map(o=>dayTripCard(st,i,o.x,o.idx)).join('')}</div>`}).join('');
 const stayCard=(st,i)=>`<article class="trip-story-stay" data-stay="${i}"><div class="trip-story-stay__head"><div><span>${String(i+1).padStart(2,'0')}</span><div><small>${i===0?(es?'PRIMER DESTINO':'FIRST DESTINATION'):(es?'SIGUIENTE DESTINO':'NEXT DESTINATION')}</small><strong>${_tripStoryEsc_(st.place|| (es?'Elige tu destino':'Choose destination'))}</strong></div></div>${i?`<button type="button" class="trip-story-remove" data-remove-stay="${i}" aria-label="${es?'Eliminar destino':'Remove destination'}">×</button>`:''}</div><div class="trip-story-card-guide"><b>${i===0?(es?'¿Dónde quieres que ITBMO empiece a planificar?':'Where should ITBMO start planning?'):(es?'Continúa tu historia con el siguiente lugar donde te quedarás.':'Continue your story with the next place you will stay.')}</b><span>${es?'Primero elige país y destino. Después define la duración, la fecha y cómo aprovechar cada día.':'Choose country and destination first. Then define duration, date and how to use each day.'}</span></div><div class="trip-story-destination-grid"><label><span>${es?'País':'Country'}</span><small>${es?'Escribe al menos 2 letras.':'Type at least 2 letters.'}</small>${countryField(st.country||_tripStoryCountryFromCode_(st.countryCode)?.label||'',st.countryCode,'data-stay-country')}</label><label><span>${es?'Destino':'Destination'}</span><small>${es?'Escribe 3 letras; si no aparece, conserva lo escrito.':'Type 3 letters; if not listed, keep what you typed.'}</small>${destinationField(st.place,'data-stay-place')}</label><label><span>${es?'¿Cuántos días estarás aquí?':'How many days will you stay?'}</span><small>${es?'Define la duración de esta estancia.':'Set the length of this stay.'}</small><select data-stay-field="days">${_tripStoryDaysOptions_(st.days)}</select></label><label><span>${es?'Primer día que quieres planificar':'First day to plan'}</span><small>${i?(es?'Se mantiene la continuidad del viaje.':'Journey continuity is preserved.'):(es?'No tiene que coincidir con tu fecha de salida.':'It does not have to match your departure date.')}</small><input type="date" data-stay-field="startDate" value="${_tripStoryEsc_(st.startDate)}" ${i?'readonly':''}></label></div>${i?`<div class="trip-story-movement"><div class="trip-story-movement__title"><span>→</span><div><small>${es?'TRASLADO ENTRE ESTANCIAS':'TRANSFER BETWEEN STAYS'}</small><b>${es?`¿Cómo llegarás desde ${_tripStoryEsc_(story.stays[i-1]?.place||'el destino anterior')}?`:'How will you get here?'}</b><em>${es?'Completa solo lo que ya conozcas.':'Complete only what you already know.'}</em></div></div><label><span>${es?'Medio de transporte':'Transport'}</span><select data-stay-field="transportMode">${_tripStoryTransportOptions_(st.transportMode)}</select></label><label><span>${es?'Hora de salida · opcional':'Departure · optional'}</span><select data-stay-field="departureTime">${_tripStoryTimeOptions_(st.departureTime)}</select></label><label><span>${es?'Hora de llegada · opcional':'Arrival · optional'}</span><select data-stay-field="arrivalTime">${_tripStoryTimeOptions_(st.arrivalTime)}</select></label></div>`:''}<div class="trip-story-days-head"><div><b>${es?'Tus días en este destino':'Your days here'}</b><small>${es?'Cada día conserva su fecha real. Ajusta las horas útiles para que ITBMO sepa cuándo puede planificar.':'Each day keeps its real date. Adjust useful hours so ITBMO knows when it can plan.'}</small></div><button type="button" data-copy-hours="${i}">${es?'Aplicar el horario del Día 1 a todos':'Apply Day 1 hours to all'}</button></div><div class="trip-story-days">${dayCards(st,i)}</div></article>`;
 const render=()=>{recalc();body.innerHTML=`<section class="trip-story-section trip-story-section--start"><div class="trip-story-section-title"><span>01</span><div><small>${es?'COMIENZA TU HISTORIA':'START YOUR STORY'}</small><h4>${es?'¿Cómo comienza tu viaje?':'How does your trip begin?'}</h4><p>${es?'Completa lo que ya sabes. Los horarios y los puntos exactos pueden quedar pendientes.':'Complete what you already know. Exact times and terminals can remain pending.'}</p></div></div><div class="trip-story-start-flow"><div class="trip-story-flow-card"><div class="trip-story-flow-card__title"><span>↗</span><div><small>${es?'TU SALIDA':'YOUR DEPARTURE'}</small><b>${es?'El punto donde comienza el viaje':'Where your journey begins'}</b></div></div><div class="trip-story-start-grid"><label><span>${es?'Fecha en que sales de tu origen':'Date you leave home'}</span><small>${es?'La fecha real en que inicia tu viaje.':'The real date your journey starts.'}</small><input type="date" data-start="date" value="${_tripStoryEsc_(story.start.date||'')}"></label><label><span>${es?'Medio principal':'Main transport'}</span><small>${es?'El que usarás para este primer traslado.':'Used for this first transfer.'}</small><select data-start="transportMode">${_tripStoryTransportOptions_(story.start.transportMode||'plane')}</select></label><label><span>${es?'Lugar de salida':'Origin'}</span><small>${es?'Ciudad, aeropuerto, estación, terminal o puerto.':'City, airport, station, terminal or port.'}</small><input data-start="origin" value="${_tripStoryEsc_(story.start.origin?.label||'')}" placeholder="${es?'Ej. San José / SJO':'e.g. San José / SJO'}"></label><label><span>${es?'Hora de salida · opcional':'Departure time · optional'}</span><small>${es?'Déjala pendiente si aún no la conoces.':'Leave it pending if unknown.'}</small><select data-start="departureTime">${_tripStoryTimeOptions_(story.start.departureTime)}</select></label></div></div><div class="trip-story-flow-connector"><span>→</span></div><div class="trip-story-flow-card"><div class="trip-story-flow-card__title"><span>⌖</span><div><small>${es?'TU LLEGADA':'YOUR ARRIVAL'}</small><b>${es?'El punto donde aterrizas o desembarcas':'Where you arrive'}</b></div></div><div class="trip-story-start-grid"><label><span>${es?'Lugar de llegada':'Arrival point'}</span><small>${es?'Ciudad, aeropuerto, estación, terminal o puerto.':'City, airport, station, terminal or port.'}</small><input data-start="arrival" value="${_tripStoryEsc_(story.start.arrival?.label||'')}" placeholder="${es?'Ej. Madrid / MAD':'e.g. Madrid / MAD'}"></label><label><span>${es?'Fecha de llegada · opcional':'Arrival date · optional'}</span><small>${es?'Puede ser distinta a tu fecha de salida.':'It may differ from your departure date.'}</small><input type="date" data-start="arrivalDate" value="${_tripStoryEsc_(story.start.arrivalDate||'')}"></label><label><span>${es?'Hora de llegada · opcional':'Arrival time · optional'}</span><small>${es?'Ayuda a calcular el tiempo realmente disponible.':'Helps calculate actual available time.'}</small><select data-start="arrivalTime">${_tripStoryTimeOptions_(story.start.arrivalTime)}</select></label></div></div></div><div class="trip-story-help"><span>✦</span><div><b>${es?'¿Todavía no tienes los boletos? Puedes continuar.':'No tickets yet? You can continue.'}</b><small>${es?'La fecha del primer destino se define por separado. ITBMO nunca asumirá que llegas el mismo día que sales.':'The first destination date is defined separately. ITBMO never assumes you arrive the same day you depart.'}</small></div></div></section><section class="trip-story-section"><div class="trip-story-section-title"><span>02</span><div><small>${es?'VIVE EL RECORRIDO':'LIVE THE JOURNEY'}</small><h4>${es?'Construye tu viaje, estancia por estancia':'Build your journey, stay by stay'}</h4><p>${es?'Cada tarjeta es un lugar donde te quedarás. Las excursiones de ida y regreso viven dentro del día correspondiente.':'Each card is a place where you will stay. Same-day trips live inside the corresponding day.'}</p></div></div><div class="trip-story-stays">${story.stays.map(stayCard).join('')}</div><button class="trip-story-add trip-story-add--destination" type="button" data-add-stay><span>＋</span><div><b>${es?'Agregar siguiente destino':'Add next destination'}</b><small>${es?'Continúa la historia exactamente en el orden en que la vivirás.':'Continue the story in the exact order you will live it.'}</small></div></button></section><section class="trip-story-section trip-story-section--finish"><div class="trip-story-section-title"><span>03</span><div><small>${es?'CIERRA EL CÍRCULO':'COMPLETE THE STORY'}</small><h4>${es?'¿Aquí termina tu viaje?':'Does your trip end here?'}</h4><p>${es?'Si ya conoces tu regreso, agrégalo. Si todavía no, puedes guardar tu historia sin esta información.':'If you know your return, add it. Otherwise you can save your story without it.'}</p></div></div><label class="trip-story-return-toggle"><input type="checkbox" data-return-enabled ${story.returnTrip.enabled?'checked':''}><span></span><div><b>${es?'Agregar información de regreso':'Add return information'}</b><small>${es?'Opcional · salida, llegada y horarios pueden completarse después':'Optional · departure, arrival and times can be completed later'}</small></div></label>${story.returnTrip.enabled?`<div class="trip-story-start-grid trip-story-return"><label><span>${es?'Fecha de salida':'Departure date'}</span><small>${es?'Día en que dejas tu último destino.':'Day you leave your last destination.'}</small><input type="date" data-return="departureDate" value="${_tripStoryEsc_(story.returnTrip.departureDate||'')}"></label><label><span>${es?'Medio de transporte':'Transport'}</span><small>${es?'Selecciona el principal.':'Choose the main one.'}</small><select data-return="transportMode">${_tripStoryTransportOptions_(story.returnTrip.transportMode||'plane')}</select></label><label><span>${es?'Lugar de salida':'Origin'}</span><small>${es?'Ciudad, aeropuerto, estación, terminal o puerto.':'City, airport, station, terminal or port.'}</small><input data-return="origin" value="${_tripStoryEsc_(story.returnTrip.origin?.label||'')}"></label><label><span>${es?'Hora de salida · opcional':'Departure · optional'}</span><small>${es?'Puede quedar pendiente.':'Can remain pending.'}</small><select data-return="departureTime">${_tripStoryTimeOptions_(story.returnTrip.departureTime)}</select></label><label><span>${es?'Lugar de llegada':'Arrival'}</span><small>${es?'Tu punto final de regreso.':'Your final return point.'}</small><input data-return="arrival" value="${_tripStoryEsc_(story.returnTrip.arrival?.label||'')}"></label><label><span>${es?'Fecha de llegada · opcional':'Arrival date · optional'}</span><small>${es?'Útil en viajes nocturnos o largos.':'Useful for overnight or long trips.'}</small><input type="date" data-return="arrivalDate" value="${_tripStoryEsc_(story.returnTrip.arrivalDate||'')}"></label><label><span>${es?'Hora de llegada · opcional':'Arrival time · optional'}</span><small>${es?'Puede quedar pendiente.':'Can remain pending.'}</small><select data-return="arrivalTime">${_tripStoryTimeOptions_(story.returnTrip.arrivalTime)}</select></label></div>`:''}</section><div class="trip-story-actions"><button type="button" class="secondary" data-story-cancel>${es?'Cancelar':'Cancel'}</button><div class="trip-story-actions__save"><small>${es?'Cuando tu historia esté lista':'When your story is ready'}</small><button type="button" class="primary" data-story-save><span>✦</span>${es?'Guardar historia de mi viaje':'Save my trip story'} →</button></div></div>`;
  body.querySelectorAll('[data-start]').forEach(el=>el.onchange=()=>{const k=el.dataset.start;if(k==='origin')story.start.origin.label=el.value;else if(k==='arrival')story.start.arrival.label=el.value;else story.start[k]=el.value;if(k==='arrivalDate')render();});
  body.querySelectorAll('[data-stay]').forEach(card=>{const i=Number(card.dataset.stay),st=story.stays[i];
   const countryInput=card.querySelector('[data-stay-country]');if(countryInput){let timer;countryInput.oninput=()=>{st.country=countryInput.value;st.countryCode='';clearTimeout(timer);const menu=countryInput.parentElement.querySelector('.trip-story-suggestions'),items=_tripStoryCountrySuggestions_(countryInput.value);if(countryInput.value.trim().length<2){menu.hidden=true;return;}menu.innerHTML=items.length?items.map(x=>`<button type="button" data-code="${_tripStoryEsc_(x.code)}">${_tripStoryEsc_(x.label)}</button>`).join(''):`<div>${es?'Escribe un país de la lista para asociar correctamente el destino.':'Type a listed country so the destination can be matched correctly.'}</div>`;menu.hidden=false;menu.querySelectorAll('button').forEach((b,n)=>b.onclick=()=>{st.countryCode=items[n].code;st.country=items[n].label;st.place='';render();});};countryInput.onchange=()=>{const m=_tripStoryCountryMatchLabel_(countryInput.value);if(m){st.countryCode=m.code;st.country=m.label;}};}
   const placeInput=card.querySelector('[data-stay-place]');if(placeInput){let timer;placeInput.oninput=()=>{st.place=placeInput.value;card.querySelector('.trip-story-stay__head strong').textContent=placeInput.value||(es?'Elige tu destino':'Choose destination');clearTimeout(timer);const menu=placeInput.parentElement.querySelector('.trip-story-suggestions');if(placeInput.value.trim().length<3||!st.countryCode){menu.hidden=true;return;}timer=setTimeout(async()=>{const q=placeInput.value.trim(),items=await _tripStorySuggestions_(st.countryCode,q);if(placeInput.value.trim()!==q)return;menu.innerHTML=items.length?items.slice(0,10).map(x=>`<button type="button">${_tripStoryEsc_(typeof x==='string'?x:x.label)}</button>`).join(''):`<div>${es?'No aparece en la lista. Puedes continuar con el nombre escrito.':'Not listed. You can continue with the name you typed.'}</div>`;menu.hidden=false;menu.querySelectorAll('button').forEach((b,n)=>b.onclick=()=>{st.place=typeof items[n]==='string'?items[n]:items[n].label;render();});},220);};}
   card.querySelectorAll('[data-stay-field]').forEach(el=>el.onchange=()=>{const k=el.dataset.stayField;if(k==='days'){st.days=Number(el.value);_tripStoryEnsureStay_(st);render();return;}if(k==='startDate'){st.startDate=el.value;st._autoDate=false;render();return;}st[k]=el.value;});
   card.querySelectorAll('[data-hour]').forEach(el=>el.onchange=()=>{st.perDay[Number(el.dataset.day)][el.dataset.hour]=el.value;});card.querySelector('[data-copy-hours]')?.addEventListener('click',()=>{const first=st.perDay[0]||_tripStoryDefaultHours_(0,st);st.perDay=Array.from({length:st.days},(_,d)=>({day:d+1,start:first.start,end:first.end}));render();});
   card.querySelectorAll('[data-dt-country]').forEach(el=>{const idx=Number(el.dataset.dtCountry),dt=st.dayTrips[idx];let timer;el.oninput=()=>{dt.country=el.value;dt.countryCode='';clearTimeout(timer);const menu=el.parentElement.querySelector('.trip-story-suggestions'),items=_tripStoryCountrySuggestions_(el.value);if(el.value.trim().length<2){menu.hidden=true;return;}menu.innerHTML=items.length?items.map(x=>`<button type="button">${_tripStoryEsc_(x.label)}</button>`).join(''):`<div>${es?'Escribe un país de la lista.':'Type a listed country.'}</div>`;menu.hidden=false;menu.querySelectorAll('button').forEach((b,n)=>b.onclick=()=>{dt.countryCode=items[n].code;dt.country=items[n].label;dt.place='';render();});};el.onchange=()=>{const m=_tripStoryCountryMatchLabel_(el.value);if(m){dt.countryCode=m.code;dt.country=m.label;}};});
   card.querySelectorAll('[data-dt-place]').forEach(el=>{const idx=Number(el.dataset.dtPlace),dt=st.dayTrips[idx];let timer;el.oninput=()=>{dt.place=el.value;clearTimeout(timer);const menu=el.parentElement.querySelector('.trip-story-suggestions');if(el.value.trim().length<3||!dt.countryCode){menu.hidden=true;return;}timer=setTimeout(async()=>{const q=el.value.trim(),items=await _tripStorySuggestions_(dt.countryCode,q);if(el.value.trim()!==q)return;menu.innerHTML=items.length?items.slice(0,10).map(x=>`<button type="button">${_tripStoryEsc_(typeof x==='string'?x:x.label)}</button>`).join(''):`<div>${es?'No aparece en la lista. Puedes conservar lo escrito.':'Not listed. You can keep what you typed.'}</div>`;menu.hidden=false;menu.querySelectorAll('button').forEach((b,n)=>b.onclick=()=>{dt.place=typeof items[n]==='string'?items[n]:items[n].label;render();});},220);};});
   card.querySelectorAll('[data-dt-out]').forEach(el=>el.onchange=()=>{const dt=st.dayTrips[Number(el.dataset.dtIndex)];dt.outbound[el.dataset.dtOut]=el.value;if(el.dataset.dtOut==='transportMode'&&!dt.return.transportMode)dt.return.transportMode=el.value;});card.querySelectorAll('[data-dt-return]').forEach(el=>el.onchange=()=>{const dt=st.dayTrips[Number(el.dataset.dtIndex)];dt.return[el.dataset.dtReturn]=el.value;});
  });
  body.querySelectorAll('[data-remove-stay]').forEach(b=>b.onclick=()=>{story.stays.splice(Number(b.dataset.removeStay),1);render();});body.querySelector('[data-add-stay]')?.addEventListener('click',()=>{const prev=story.stays.at(-1),start=prev?.startDate?_tripStoryAddDays_(prev.startDate,prev.days):'';story.stays.push(_tripStoryEnsureStay_({startDate:start,_autoDate:true,days:1}));render();});body.querySelectorAll('[data-add-dt]').forEach(b=>b.onclick=()=>{const st=story.stays[Number(b.dataset.addDt)];st.dayTrips.push(_tripStoryEnsureDayTrip_({day:Number(b.dataset.day),countryCode:st.countryCode,country:st.country,place:'',outbound:{transportMode:'recommend'},return:{transportMode:'recommend'}}));render();});body.querySelectorAll('[data-remove-dt]').forEach(b=>b.onclick=()=>{const [i,j]=b.dataset.removeDt.split(':').map(Number);story.stays[i].dayTrips.splice(j,1);render();});
  body.querySelector('[data-return-enabled]')?.addEventListener('change',e=>{story.returnTrip.enabled=e.target.checked;render();});body.querySelectorAll('[data-return]').forEach(el=>el.onchange=()=>{const k=el.dataset.return;if(k==='origin')story.returnTrip.origin={...(story.returnTrip.origin||{}),label:el.value};else if(k==='arrival')story.returnTrip.arrival={...(story.returnTrip.arrival||{}),label:el.value};else story.returnTrip[k]=el.value;});body.querySelector('[data-story-cancel]').onclick=close;
  body.querySelector('[data-story-save]').onclick=async()=>{if(!story.start.date){alert(es?'Indica la fecha en que comienza tu viaje.':'Enter your trip start date.');return;}if(!story.stays.length||story.stays.some(x=>!x.countryCode||!String(x.place||'').trim()||!x.startDate)){alert(es?'Completa país, destino y fecha del primer día de cada estancia.':'Complete country, destination and first-day date for each stay.');return;}const total=story.stays.reduce((n,x)=>n+Number(x.days||1),0);if(total>MAX_TRIP_STORY_DAYS){alert(es?`El viaje puede tener hasta ${MAX_TRIP_STORY_DAYS} días en esta versión.`:`This version supports trips up to ${MAX_TRIP_STORY_DAYS} days.`);return;}for(const st of story.stays){for(const d of st.perDay){if(d.start>=d.end){alert(es?`Revisa el horario del Día ${d.day} en ${st.place}.`:`Review Day ${d.day} hours in ${st.place}.`);return;}}for(const dt of st.dayTrips){if(!String(dt.place||'').trim()){alert(es?`Completa el destino de la excursión del Día ${dt.day} en ${st.place}.`:`Complete the Day ${dt.day} excursion destination in ${st.place}.`);return;}}}story.ended=true;_travelV2()?.setTripStory?.(story);applyTripStoryToCompatibility(story);renderTripStorySummary();close();await saveDestinations({showReadyModal:true});};
 };if(!story.stays.length)story.stays.push(_tripStoryEnsureStay_({startDate:story.start.arrivalDate||'',days:1}));render();
}
function applyTripStoryToCompatibility(story){
 const stays=story?.stays||[];if(!stays.length)return;const totalDays=stays.reduce((n,x)=>n+Number(x.days||1),0),first=stays[0],firstCountry=_tripStoryCountryFromCode_(first.countryCode);$cityList.innerHTML='';addCityRow({city:first.place,country:firstCountry?.label||first.country||'',days:totalDays,baseDate:_tripStoryDMY_(first.startDate)});const row=qs('.city-row',$cityList);if(!row)return;const segments=[];let prev=first;
 stays.forEach((st,i)=>{if(i>0){segments.push({id:`story_move_${i}_${Date.now()}`,origin:prev.place,destination:st.place,departureDate:st.startDate,arrivalDate:st.startDate,departureTime:st.departureTime||'',arrivalTime:st.arrivalTime||'',transportMode:st.transportMode||'recommend',timePrecision:(st.departureTime&&st.arrivalTime)?'exact':'unknown',disposition:'continue',nights:Number(st.days||1),source:'TRIP_STORY'});} (st.dayTrips||[]).forEach((raw,j)=>{const dt=_tripStoryEnsureDayTrip_(raw);if(!dt.place)return;const date=_tripStoryAddDays_(st.startDate,Math.max(0,Number(dt.day||1)-1));segments.push({id:`story_daytrip_${i}_${j}_${Date.now()}`,origin:st.place,destination:dt.place,departureDate:date,arrivalDate:date,departureTime:dt.outbound.departureTime||'',arrivalTime:dt.outbound.arrivalTime||'',returnDepartureDate:date,returnDepartureTime:dt.return.departureTime||'',returnArrivalDate:date,returnArrivalTime:dt.return.arrivalTime||'',returnDestination:st.place,transportMode:dt.outbound.transportMode||'recommend',returnTransportMode:dt.return.transportMode||dt.outbound.transportMode||'recommend',timePrecision:(dt.outbound.departureTime&&dt.outbound.arrivalTime&&dt.return.departureTime&&dt.return.arrivalTime)?'exact':'unknown',disposition:'roundtrip',source:'TRIP_STORY_DAYTRIP'});});prev=st;});
 _travelV2()?.setRouteSegments?.(row,segments);const days=qs('.days',row);if(days){days.innerHTML=Array.from({length:MAX_TRIP_STORY_DAYS},(_,i)=>`<option value="${i+1}">${i+1}</option>`).join('');days.value=String(totalDays);days.dispatchEvent(new Event('change',{bubbles:true}));}const base=qs('.baseDate',row);if(base)base.value=_tripStoryDMY_(first.startDate);const perDay=stays.flatMap(st=>st.perDay.map((d,i)=>({...d,date:_tripStoryDayDate_(st,i),physicalDestination:st.place})));qsa('.hours-day',row).forEach((hd,idx)=>{if(!perDay[idx])return;const a=qs('.start',hd),b=qs('.end',hd);if(a)a.value=perDay[idx].start;if(b)b.value=perDay[idx].end;});_travelV2()?.renderRowSummary?.(row);updateAddCityButtonState();updateTravelBuilderProgress();
}
