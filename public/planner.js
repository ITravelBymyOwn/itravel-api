...
/* =========================================================
   ITBMO V2.8.0 · TRIP STORY EDITOR / CONTINUOUS JOURNEY
...
function openTripStoryBuilder(){
 const es=getLang()==='es',story=JSON.parse(JSON.stringify(_tripStoryCurrent_()));story.schema_version=4;story.start=story.start||{};story.start.origin=story.start.origin||{label:'',type:'city'};story.start.arrival=story.start.arrival||{label:'',type:'city'};story.stays=(story.stays||[]).map(_tripStoryEnsureStay_);story.returnTrip=story.returnTrip||{enabled:false,origin:{},arrival:{}};let activeStay=Math.max(0,story.stays.length-1),dragStay=null,dragDayTrip=null;
