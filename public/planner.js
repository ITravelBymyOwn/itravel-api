...
/* =========================================================
   ITBMO V2.7 · TRIP STORY / CONTINUOUS JOURNEY BUILDER
   The user describes one chronological story. The legacy city-row remains an
   internal compatibility shell only; generation receives the richer Trip Story
   through travelModelV2 and its deterministic route segments.
========================================================= */
function _tripStoryEsc_(v=''){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));}
function _tripStoryISO_(v=''){const m=String(v).match(/^(\d{4})-(\d{2})-(\d{2})$/);return m?v:'';}
...
function _tripStoryLocationTypeOptions_(selected='city'){const es=getLang()==='es';return [['city',es?'Ciudad / lugar':'City / place'],['airport',es?'Aeropuerto':'Airport'],['rail_station',es?'Estación de tren / metro':'Rail / metro station'],['bus_station',es?'Estación / terminal de bus':'Bus station / terminal'],['port',es?'Puerto / ferry':'Port / ferry'],['other',es?'Otro punto':'Other location']].map(([v,l])=>`<option value="${v}" ${v===selected?'selected':''}>${l}</option>`).join('');}
function _tripStoryCurrent_(){return _travelV2()?.state?.tripStory || {schema_version:1,start:{date:'',transportMode:'plane',origin:{label:'',type:'city'},arrival:{label:'',type:'city'},departureTime:'',arrivalTime:''},stays:[],returnTrip:{enabled:false,transportMode:'plane',origin:{label:'',type:'city'},arrival:{label:'',type:'city'},departureDate:'',departureTime:'',arrivalDate:'',arrivalTime:''},ended:false};}
function renderTripStorySummary(){const host=qs('#trip-story-summary');if(!host)return;const story=_tripStoryCurrent_(),stays=story.stays||[];if(!stays.length){host.hidden=true;return;}host.hidden=false;const es=getLang()==='es';const nodes=[];if(story.start?.origin?.label)nodes.push(`<div class="trip-story-node"><b>${_tripStoryEsc_(story.start.origin.label)}</b><small>${es?'Inicio del viaje':'Trip start'}${story.start.date?` · ${_tripStoryDMY_(story.start.date)}`:''}</small></div>`);stays.forEach((x,i)=>{if(nodes.length)nodes.push('<div class="trip-story-arrow">→</div>');nodes.push(`<div class="trip-story-node"><b>${_tripStoryEsc_(x.place)}</b><small>${Number(x.days||1)} ${es?'día(s)':'day(s)'} · ${_tripStoryDMY_(x.startDate)}</small></div>`)});if(story.returnTrip?.enabled&&story.returnTrip?.arrival?.label){nodes.push('<div class="trip-story-arrow">→</div>',`<div class="trip-story-node"><b>${_tripStoryEsc_(story.returnTrip.arrival.label)}</b><small>${es?'Regreso':'Return'}</small></div>`)}host.innerHTML=`<div class="trip-story-summary__head"><strong>${es?'Tu viaje, en orden':'Your trip, in order'}</strong><button type="button" id="edit-trip-story">${es?'Editar historia':'Edit story'}</button></div><div class="trip-story-timeline">${nodes.join('')}</div>`;qs('#edit-trip-story')?.addEventListener('click',openTripStoryBuilder);}
function openTripStoryBuilder(){
 const es=getLang()==='es', story=JSON.parse(JSON.stringify(_tripStoryCurrent_())); if(!story.stays)story.stays=[];
 const overlay=document.createElement('div');overlay.className='trip-story-overlay';overlay.innerHTML=`<div class="trip-story-modal" role="dialog" aria-modal="true"><div class="trip-story-modal__head"><div><span class="trip-story-kicker">${es?'LA HISTORIA DE TU VIAJE':'YOUR TRIP STORY'}</span><h3>${es?'Construye tu viaje, de principio a fin':'Build your trip, from start to finish'}</h3><p>${es?'Cuéntanos la secuencia tal como la vivirás. Los horarios de transporte son opcionales: si aún no los tienes, continúa y podrás completarlos después.':'Tell us the sequence exactly as you will live it. Transport times are optional; continue if you do not know them yet.'}</p></div><button class="trip-story-close" type="button">×</button></div><div data-story-body></div></div>`;document.body.appendChild(overlay);
 ...
 const recalc=()=>{let cursor=story.start.date||'';(story.stays||[]).forEach((st,i)=>{st.days=Math.max(1,Math.min(30,Number(st.days||1)));st.startDate=cursor||st.startDate||'';if(cursor)cursor=_tripStoryAddDays_(cursor,st.days);st.sequence=i+1;});};
 const render=()=>{recalc();body.innerHTML=`<section class="trip-story-section"><h4>${es?'1. ¿Cómo comienza tu viaje?':'1. How does your trip begin?'}</h4><p>${es?'La fecha es el día en que comienza tu desplazamiento desde el origen. Puedes indicar una ciudad o ser tan preciso como un aeropuerto, estación, terminal o puerto.':'The date is when you begin traveling from your origin. Use a city or a precise airport, station, terminal or port.'}</p><div class="trip-story-grid three"><label>${es?'Fecha de inicio':'Start date'}<input type="date" data-start="date" value="${_tripStoryEsc_(story.start.date||'')}"></label><label>${es?'Medio principal':'Main transport'}<select data-start="transportMode">${_tripStoryTransportOptions_(story.start.transportMode)}</select></label><label>${es?'Tipo de salida':'Origin type'}<select data-start="originType">${_tripStoryLocationTypeOptions_(story.start.origin?.type)}</select></label></div><div class="trip-story-grid"><label>${es?'Lugar de salida':'Origin'}<input data-start="origin" value="${_tripStoryEsc_(story.start.origin?.label||'')}" placeholder="${es?'Ciudad, aeropuerto, estación, terminal o puerto':'City, airport, station, terminal or port'}"></label><label>${es?'Lugar de llegada':'Arrival location'}<input data-start="arrival" value="${_tripStoryEsc_(story.start.arrival?.label||'')}" placeholder="${es?'Ciudad, aeropuerto, estación, terminal o puerto':'City, airport, station, terminal or port'}"></label><label>${es?'Hora de salida · opcional':'Departure time · optional'}<input type="time" data-start="departureTime" value="${_tripStoryEsc_(story.start.departureTime||'')}"></label><label>${es?'Hora de llegada · opcional':'Arrival time · optional'}<input type="time" data-start="arrivalTime" value="${_tripStoryEsc_(story.start.arrivalTime||'')}"></label></div><div class="trip-story-help">${es?'¿No tienes boletos u horarios todavía? No hay problema. Los lugares pueden ser generales y las horas pueden quedar vacías.':'No tickets or times yet? No problem. Locations can be general and times may stay blank.'}</div></section><section class="trip-story-section"><h4>${es?'2. Destinos que quieres que ITBMO planifique':'2. Places you want ITBMO to plan'}</h4><p>${es?'Cada estancia continúa automáticamente desde la anterior. Si duermes en otro lugar, agrégalo como siguiente destino. Una excursión que regresa el mismo día se agrega dentro del día correspondiente.':'Each stay continues automatically from the previous one. If you sleep somewhere else, add it as the next destination. Same-day round trips belong to that day.'}</p><div class="trip-story-stays">${story.stays.map((st,i)=>`<article class="trip-story-stay" data-stay="${i}"><div class="trip-story-stay__head"><strong>${String(i+1).padStart(2,'0')} · ${_tripStoryEsc_(st.place|| (es?'Nuevo destino':'New destination'))}</strong>${i?`<button type="button" class="trip-story-remove" data-remove-stay="${i}">×</button>`:''}</div><div class="trip-story-grid three"><label>${es?'Destino / lugar':'Destination / place'}<input data-stay-field="place" value="${_tripStoryEsc_(st.place||'')}"></label><label>${es?'Número de días':'Number of days'}<input type="number" min="1" max="30" data-stay-field="days" value="${Number(st.days||1)}"></label><label>${es?'Inicio calculado':'Calculated start'}<input value="${_tripStoryEsc_(_tripStoryDMY_(st.startDate)||'')}" disabled></label></div><div class="trip-story-grid three">${i?`<label>${es?'Cómo llegas desde el destino anterior':'How you arrive from previous stay'}<select data-stay-field="transportMode">${_tripStoryTransportOptions_(st.transportMode||'recommend')}</select></label><label>${es?'Hora de salida · opcional':'Departure time · optional'}<input type="time" data-stay-field="departureTime" value="${_tripStoryEsc_(st.departureTime||'')}"></label><label>${es?'Hora de llegada · opcional':'Arrival time · optional'}<input type="time" data-stay-field="arrivalTime" value="${_tripStoryEsc_(st.arrivalTime||'')}"></label>`:`<label>${es?'Comenzar a planificar':'Start planning'}<select data-stay-field="planningStart"><option value="same" ${st.planningStart!=='next'?'selected':''}>${es?'El mismo día':'Same day'}</option><option value="next" ${st.planningStart==='next'?'selected':''}>${es?'Al día siguiente':'Next day'}</option></select></label>`}</div><div class="trip-story-daytrips">${(st.dayTrips||[]).map((dt,j)=>`<div class="trip-story-daytrip"><label>${es?'Día':'Day'}<select data-daytrip="day" data-dt="${j}">${Array.from({length:Number(st.days||1)},(_,d)=>`<option value="${d+1}" ${Number(dt.day)===d+1?'selected':''}>${es?'Día':'Day'} ${d+1}</option>`).join('')}</select></label><label>${es?'Excursión ida y regreso':'Same-day excursion'}<input data-daytrip="place" data-dt="${j}" value="${_tripStoryEsc_(dt.place||'')}"></label><label>${es?'Transporte':'Transport'}<select data-daytrip="transportMode" data-dt="${j}">${_tripStoryTransportOptions_(dt.transportMode||'recommend')}</select></label><button class="trip-story-remove" type="button" data-remove-dt="${j}">×</button></div>`).join('')}</div><button class="trip-story-add" type="button" data-add-dt="${i}">＋ ${es?'Agregar excursión de un día':'Add same-day excursion'}</button></article>`).join('')}</div><button class="trip-story-add" type="button" data-add-stay>＋ ${es?'Agregar siguiente destino':'Add next destination'}</button></section><section class="trip-story-section"><h4>${es?'3. ¿Aquí termina tu viaje?':'3. Does your trip end here?'}</h4><p>${es?'El regreso es opcional. Si ya conoces cómo vuelves, agrégalo; también puedes terminar la historia sin esta información.':'Return information is optional. Add it if known, or finish the story without it.'}</p><label style="flex-direction:row;align-items:center"><input style="width:auto;min-height:auto" type="checkbox" data-return-enabled ${story.returnTrip?.enabled?'checked':''}> ${es?'Agregar información de regreso':'Add return information'}</label>${story.returnTrip?.enabled?`<div class="trip-story-grid three" style="margin-top:12px"><label>${es?'Fecha de salida':'Departure date'}<input type="date" data-return="departureDate" value="${_tripStoryEsc_(story.returnTrip.departureDate||'')}"></label><label>${es?'Medio':'Transport'}<select data-return="transportMode">${_tripStoryTransportOptions_(story.returnTrip.transportMode||'plane')}</select></label><label>${es?'Lugar de salida':'Origin'}<input data-return="origin" value="${_tripStoryEsc_(story.returnTrip.origin?.label||'')}"></label><label>${es?'Hora de salida · opcional':'Departure time · optional'}<input type="time" data-return="departureTime" value="${_tripStoryEsc_(story.returnTrip.departureTime||'')}"></label><label>${es?'Lugar de llegada':'Arrival'}<input data-return="arrival" value="${_tripStoryEsc_(story.returnTrip.arrival?.label||'')}"></label><label>${es?'Fecha de llegada · opcional':'Arrival date · optional'}<input type="date" data-return="arrivalDate" value="${_tripStoryEsc_(story.returnTrip.arrivalDate||'')}"></label><label>${es?'Hora de llegada · opcional':'Arrival time · optional'}<input type="time" data-return="arrivalTime" value="${_tripStoryEsc_(story.returnTrip.arrivalTime||'')}"></label></div>`:''}</section><div class="trip-story-actions"><button type="button" class="secondary" data-story-cancel>${es?'Cancelar':'Cancel'}</button><button type="button" class="primary" data-story-save>${es?'Guardar historia de mi viaje →':'Save my trip story →'}</button></div>`;
  body.querySelectorAll('[data-start]').forEach(el=>el.onchange=()=>{const k=el.dataset.start;if(k==='originType')story.start.origin.type=el.value;else if(k==='origin')story.start.origin.label=el.value;else if(k==='arrival')story.start.arrival.label=el.value;else story.start[k]=el.value;if(k==='date')render();});
  body.querySelectorAll('[data-stay]').forEach(card=>card.querySelectorAll('[data-stay-field]').forEach(el=>el.onchange=()=>{const st=story.stays[Number(card.dataset.stay)],k=el.dataset.stayField;st[k]=el.value;if(k==='days'||k==='place')render();}));
  body.querySelectorAll('[data-remove-stay]').forEach(b=>b.onclick=()=>{story.stays.splice(Number(b.dataset.removeStay),1);render();});body.querySelector('[data-add-stay]')?.addEventListener('click',()=>{story.stays.push({place:'',days:1,transportMode:'recommend',departureTime:'',arrivalTime:'',planningStart:'same',dayTrips:[]});render();});
  body.querySelectorAll('[data-add-dt]').forEach(b=>b.onclick=()=>{const st=story.stays[Number(b.dataset.addDt)];st.dayTrips=st.dayTrips||[];st.dayTrips.push({day:1,place:'',transportMode:'recommend'});render();});body.querySelectorAll('[data-remove-dt]').forEach(b=>b.onclick=()=>{const card=b.closest('[data-stay]'),st=story.stays[Number(card.dataset.stay)];st.dayTrips.splice(Number(b.dataset.removeDt),1);render();});body.querySelectorAll('[data-daytrip]').forEach(el=>el.onchange=()=>{const st=story.stays[Number(el.closest('[data-stay]').dataset.stay)],dt=st.dayTrips[Number(el.dataset.dt)];dt[el.dataset.daytrip]=el.dataset.daytrip==='day'?Number(el.value):el.value;});
  body.querySelector('[data-return-enabled]')?.addEventListener('change',e=>{story.returnTrip.enabled=e.target.checked;render();});body.querySelectorAll('[data-return]').forEach(el=>el.onchange=()=>{const k=el.dataset.return;if(k==='origin')story.returnTrip.origin.label=el.value;else if(k==='arrival')story.returnTrip.arrival.label=el.value;else story.returnTrip[k]=el.value;});body.querySelector('[data-story-cancel]').onclick=close;body.querySelector('[data-story-save]').onclick=()=>{if(!story.start.date||!story.stays.length||story.stays.some(x=>!String(x.place||'').trim())){alert(es?'Indica la fecha de inicio y al menos un destino válido.':'Enter the start date and at least one valid destination.');return;}const total=story.stays.reduce((n,x)=>n+Number(x.days||1),0);if(total>MAX_TRIP_STORY_DAYS){alert(es?`El viaje puede tener hasta ${MAX_TRIP_STORY_DAYS} días en esta versión.`:`This version supports trips up to ${MAX_TRIP_STORY_DAYS} days.`);return;}story.ended=true;_travelV2()?.setTripStory?.(story);applyTripStoryToCompatibility(story);renderTripStorySummary();close();};
 }; if(!story.stays.length)story.stays.push({place:story.start?.arrival?.label||'',days:1,planningStart:'same',transportMode:'recommend',departureTime:'',arrivalTime:'',dayTrips:[]});render();
}
function applyTripStoryToCompatibility(story){
 const stays=story?.stays||[];if(!stays.length)return;const totalDays=stays.reduce((n,x)=>n+Number(x.days||1),0);$cityList.innerHTML='';addCityRow({city:stays[0].place,country:'',days:totalDays,baseDate:_tripStoryDMY_(story.start.date)});const row=qs('.city-row',$cityList);if(!row)return;const segments=[];let cursor=story.start.date,prev=stays[0];
 // Initial arrival is context, not an invented intercity itinerary row.
 stays.forEach((st,i)=>{st.startDate=cursor;if(i>0){segments.push({id:`story_move_${i}_${Date.now()}`,origin:prev.place,destination:st.place,departureDate:cursor,arrivalDate:cursor,departureTime:st.departureTime||'',arrivalTime:st.arrivalTime||'',transportMode:st.transportMode||'recommend',timePrecision:(st.departureTime&&st.arrivalTime)?'exact':'unknown',disposition:'continue',nights:Number(st.days||1),source:'TRIP_STORY'});} (st.dayTrips||[]).forEach((dt,j)=>{if(!dt.place)return;const date=_tripStoryAddDays_(cursor,Math.max(0,Number(dt.day||1)-1));segments.push({id:`story_daytrip_${i}_${j}_${Date.now()}`,origin:st.place,destination:dt.place,departureDate:date,arrivalDate:date,departureTime:'',arrivalTime:'',returnDepartureDate:date,returnArrivalTime:'',returnDestination:st.place,transportMode:dt.transportMode||'recommend',returnTransportMode:dt.transportMode||'recommend',timePrecision:'unknown',disposition:'roundtrip',source:'TRIP_STORY_DAYTRIP'});});cursor=_tripStoryAddDays_(cursor,Number(st.days||1));prev=st;});
 _travelV2()?.setRouteSegments?.(row,segments);const days=qs('.days',row);if(days){days.innerHTML=Array.from({length:MAX_TRIP_STORY_DAYS},(_,i)=>`<option value="${i+1}">${i+1}</option>`).join('');days.value=String(totalDays);days.dispatchEvent(new Event('change',{bubbles:true}));}_travelV2()?.renderRowSummary?.(row);updateAddCityButtonState();updateTravelBuilderProgress();
}
