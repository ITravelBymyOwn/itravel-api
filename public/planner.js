...
`.trim();
  ...
  // ✅ FIX ultraquirúrgico: aceptar también Formato A (city_day) para evitar fallback por idioma
  const hasAny =
    parsed && (
      parsed.rows ||
      parsed.destinations ||
  ...
  if(hasAny){
    ...
    // ✅ FIX ultraquirúrgico: si viene city_day, aplanar a rows
    else if(Array.isArray(parsed.city_day)){
      ...
      tmpRows = flat.map(r=>normalizeRow(r));
    }
