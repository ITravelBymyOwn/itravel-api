    ...
    dayContexts.forEach(ctx=>{
      ...
      dayMoves.forEach(m=>{
        if(m.departureDate===ctx.date){
          ...
          ctx.fixed_transfers.push({transfer_id:`${m.id}:out`,direction:m.disposition==='roundtrip'?'daytrip_out':'main',user_fixed:true,origin:m.origin,destination:m.destination,departure:m.departureTime||null,arrival:m.arrivalTime||null,date:m.departureDate,time_precision:m.timePrecision||'unknown',source:m.source||'USER_FIXED',user_fixed:m.source!=='ROUTE_ESTIMATED',route_estimated:m.source==='ROUTE_ESTIMATED',route_resolution:m.routeResolution||null,mode:m.transportMode||null});
          if(exact)ctx.location_windows.push({location:`${m.origin} → ${m.destination}`,start:m.departureTime,end:m.arrivalTime,type:'fixed_transfer',transfer_id:`${m.id}:out`,user_fixed:true});
        ...
        if(m.disposition==='roundtrip'&&m.returnDepartureDate===ctx.date){
          ...
          ctx.fixed_transfers.push({transfer_id:`${m.id}:return`,direction:'daytrip_return',user_fixed:true,origin:m.destination,destination:m.origin,departure:m.returnDepartureTime||null,arrival:m.returnArrivalTime||null,date:m.returnDepartureDate,time_precision:m.timePrecision||'unknown',source:m.source||'USER_FIXED',user_fixed:m.source!=='ROUTE_ESTIMATED',route_estimated:m.source==='ROUTE_ESTIMATED',route_resolution:m.routeResolution||null,mode:m.returnTransportMode||m.transportMode||null});
          if(exactRet)ctx.location_windows.push({location:`${m.destination} → ${m.origin}`,start:m.returnDepartureTime,end:m.returnArrivalTime,type:'fixed_transfer',transfer_id:`${m.id}:return`,user_fixed:true});
